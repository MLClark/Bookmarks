...
--,pbe.monthlyamount as total_cost
--,cast(pboc.employeerate as dec(18,2)) as total_cost
,cast(pbe.monthlyamount * 100 as integer) as total_cost
...
,case when pbe.benefitelection = 'T' then to_char(pbe.effectivedate,'YYYYMMDD') end ::char(8) as term_stat_chg_date
...
join ( select personid, min(effectivedate) as effectivedate
