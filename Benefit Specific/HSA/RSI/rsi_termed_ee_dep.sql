...
,case when pbehra.benefitsubclass = '1Y' and bcdhra.benefitcoveragedescshort = 'Empl Only' then 'Single'
      ...
      else 'EE+1' end ::char(30) as enroll_cvg_type_id
...
left join person_bene_election pbemed
 ...
 and pbemed.personid in (select distinct personid from person_bene_election where current_timestamp between createts and endts 
...
left join person_bene_election pbehra
  on pbehra.personid = pbemed.personid 
 and pbehra.benefitelection <> 'W'
 ...
 and pbehra.personid in (select distinct personid from person_bene_election where current_timestamp between createts and endts 
                         and benefitsubclass in ('1Y') and benefitelection = 'E' and selectedoption = 'Y' and effectivedate < enddate)    
...
-- AND pdr.dependentrelationship in ('SP','C','NA','S','DP','D','NC','NS','ND')   
