...
,case when pdr.dependentrelationship in ('NA','DP','SP') then 'Spouse'
      when pdr.dependentrelationship in ('S','D','C','ND','NS','NC')  then 'Child' 
...
JOIN person_bene_election pbe 
 ...
 and pbe.benefitsubclass in ('10','11','14','15','16','17','6Z')
 AND current_date between pbe.effectivedate AND pbe.enddate 
 AND current_timestamp between pbe.createts AND pbe.endts
  -- to be qualified for cobra the emp must have participated in employer's health care plan  
 and pbe.personid in (select distinct personid from person_bene_election where current_timestamp between createts and endts 
 ---- benefitsubclass 15,16,17 for domestic partner benefits 
