...
,case when bpd.benefitplandesc = 'Medical Flexible Spending' then 'DBI Medical FSA'
      ...
      when lc.locationcode = '903' and bpd.benefitplandesc = 'HMO Blue NE Enhanced' then 'BCBS MA Med HMO Blue NE Enhncd Value Br at Westford'
      ...
      when lc.locationcode = '903' and bpd.benefitplandesc = 'Medical Enhanced Value - Domestic Partner' then 'BCBS MA Med HMO Blue NE Enhncd Value Br at Westford'
...
JOIN (select personid, benefitsubclass, effectivedate, enddate, benefitcoverageid, eventdate
  from person_bene_election 
 where benefitsubclass in ('10','11','14')
   and enddate < '2199-12-30'
   and date_part('year',enddate)=date_part('year',current_date)) pbee on pbee.personid = pe.personid


JOIN person_bene_election pbe 
  on pbe.personid = pbee.personid 
 and pbe.selectedoption = 'Y' 
 and pbe.benefitelection = 'E'
 --and pbe.benefitactioncode = 'A'
 and pbe.benefitsubclass = pbee.benefitsubclass
 and pbe.effectivedate = pbee.effectivedate
 and pbe.benefitcoverageid = pbee.benefitcoverageid
