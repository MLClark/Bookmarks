...
,case when pdr.dependentrelationship in ('S','D','C') 
      then left(pid.identity,3)||'-'||substring(pid.identity,4,2)||'-'||right(pid.identity,4) else null end ::char(11) as dp_ssn
,case when pdr.dependentrelationship in ('S','D','C') then pnd.fname else null end ::varchar(40) as dp_fname    
,case when pdr.dependentrelationship in ('S','D','C') then pnd.lname else null end ::varchar(40) as dp_lname  
,case when pdr.dependentrelationship in ('S','D','C') then pnd.mname else null end ::char(1) as dp_mname  
,case when pdr.dependentrelationship in ('S','D','C') then to_char(pvd.birthdate,'mm/dd/yyyy') else null end ::char(10) as dp_dob
