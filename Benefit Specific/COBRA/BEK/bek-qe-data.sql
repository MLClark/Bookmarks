...
,to_char(pe.effectivedate,'mm/dd/yyyy')::char(10) as q_event_date
...
(SELECT personid, MAX(effectivedate) AS effectivedate, MAX(enddate) AS enddate, max(eventdate) as eventdate,
        RANK() OVER (PARTITION BY personid ORDER BY MAX(effectivedate) DESC) AS RANK
...
(SELECT personid, MAX(effectivedate) AS effectivedate, MAX(enddate) AS enddate, max(eventdate) as eventdate,
        ...
        WHERE enddate > effectivedate 
          ...
          and selectedoption = 'Y'
          GROUP BY personid ) as rankmedt on rankmedt.personid = pe.personid and rankmedt.rank = 1            
...
(SELECT personid, MAX(effectivedate) AS effectivedate, MAX(enddate) AS enddate, max(eventdate) as eventdate,
        ...
        WHERE enddate > effectivedate 
          ...
          and benefitelection in ( 'T')
          and selectedoption = 'Y'
          GROUP BY personid ) as rankdntt on rankdntt.personid = pe.personid and rankdntt.rank = 1            
...
(SELECT personid, MAX(effectivedate) AS effectivedate, MAX(enddate) AS enddate, max(eventdate) as eventdate,
        ...
        WHERE enddate > effectivedate 
          ...
          and benefitelection in ( 'T')
          and selectedoption = 'Y'
          GROUP BY personid ) as rankvsnt on rankvsnt.personid = pe.personid and rankvsnt.rank = 1            
...
(SELECT personid, MAX(effectivedate) AS effectivedate, MAX(enddate) AS enddate, max(eventdate) as eventdate,
        ...
        WHERE enddate > effectivedate 
          ...
          and benefitelection in ('T')
          and selectedoption = 'Y'
          GROUP BY personid ) as rankfsat on rankfsat.personid = pe.personid and rankfsat.rank = 1  
...
,'Loss of Dependent Status' ::varchar(40) as q_event_type
,to_char(deeff.enddate,'mm/dd/yyyy')::char(10) as q_event_date

----- spouse 
,' ' ::char(11) as spssn
,' ' ::varchar(40) as sp_fname    
,' ' ::varchar(40) as sp_lname  
,' ' ::char(1) as sp_mname  
,' ' ::char(10) as sp_dob
...
left join dependent_enrollment deeff
  on deeff.dependentid = de.dependentid
 and deeff.effectivedate < deeff.enddate
 and current_timestamp between deeff.createts and deeff.endts
 
left JOIN 
(select distinct pdr.personid, pdr.dependentid, rank() over(partition by pdr.personid order by  pdr.dependentid asc) as depid
   from person_dependent_relationship pdr 
  WHERE CURRENT_DATE BETWEEN pdr.EFFECTIVEDATE AND pdr.ENDDATE
    AND CURRENT_TIMESTAMP BETWEEN pdr.CREATETS AND pdr.ENDTS) rankdep on rankdep.personid = pbe.personid and rankdep.dependentid = de.dependentid
