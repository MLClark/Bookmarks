...
---- start dental 11
,case when pbe.benefitsubclass in ('3D') then 'LT' else '  ' end ::char(2) as ltd_covcode
,case when pbe.benefitsubclass in ('3D') then to_char(pbe.effectivedate,'MMDDYYYY') else '        ' end ::char(8) as ltd_cov_start
,case when pbe.benefitsubclass in ('3D') and pbe.benefitelection = 'T' then to_char(pbe.effectivedate,'MMDDYYYY') else '        ' end ::char(8) as ltd_cov_end
,case when pbe.benefitsubclass in ('3D') then '5923955' else  '       ' end ::char(7) as ltd_grp_nbr
,case when pbe.benefitsubclass in ('3D') then '0001' else   '    '  end ::char(4) as ltd_sub_cd
,case when pbe.benefitsubclass in ('3D') and cpbp.cobraplan = 'Y' and pe.emplstatus <> 'A' then '1100' 
      when pbe.benefitsubclass in ('3D') then '0001' else  '    '  end ::char(4) as ltd_branch
,'  ' ::char(2) as ltd_filler1
,case when pbe.benefitsubclass in ('3D') and pbe.benefitelection = 'E' then 'A' 
      when pbe.benefitsubclass in ('3D') and pbe.benefitelection = 'R' then 'R' else  ' ' end ::char(1) as ltd_status
,'1' ::char(1) AS ltd_mbrs_covered
,' ' ::char(10) as ltf_filler2
,to_char(pbe.monthlyamount,'FM00000000') ::char(8) as ltd_monthly_benefit_amt
