...
,case when pu.payunitid = '1' then 'Weekly'
      when pu.payunitid = '2' then 'Bi-Weekly' 
...
left join person_identity pip
 ...
 --and current_date between pip.effectivedate and pip.enddate
...
JOIN person_bene_election pbe_depfsa  
  ON pbe_depfsa.personid = pI.personid
  and pbe_depfsa.selectedoption = 'Y' 
 AND pbe_depfsa.benefitsubclass in ('60','61' )
 and pbe_depfsa.coverageamount > 0 --
 AND pbe_depfsa.benefitelection IN ('E','T')
 AND pbe_depfsa.enddate = '2199-12-31' 
 AND current_date BETWEEN pbe_depfsa.effectivedate AND pbe_depfsa.enddate 
 AND current_timestamp BETWEEN pbe_depfsa.createts AND pbe_depfsa.endts
 and pbe_depfsa.personid in (select distinct personid from person_bene_election where current_timestamp between createts and endts 
