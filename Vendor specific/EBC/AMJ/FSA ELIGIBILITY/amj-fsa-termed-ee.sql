...
,to_char(coalesce(pbemfsa.effectivedate,pbedfsa.effectivedate),'yyyy-mm-dd') ::char(10) as eff_date
,to_char(coalesce(pbemfsa.enddate,pbedfsa.enddate),'yyyy-mm-dd') ::char(10) as end_date
,case when pbemfsa.benefitsubclass = '60' then 'MED'
      when pbedfsa.benefitsubclass = '61' then 'DEP'
      end ::varchar(20) as plan_type
,to_char(coalesce(pbemfsa.coverageamount,pbedfsa.coverageamount,0),'999999.99') as election
...
join person_bene_election pbe
 ...
 and pbe.personid in (select distinct personid from person_bene_election where current_timestamp between createts and endts and effectivedate < enddate
                         and benefitsubclass in ('60','61') and benefitelection = 'E' and selectedoption = 'Y' and date_part('year',deductionstartdate)=date_part('year',current_date))
...
left join   (select personid, benefitelection, benefitcoverageid, benefitsubclass, benefitplanid, personbeneelectionpid, coverageamount, max(effectivedate) as effectivedate, max(enddate) as enddate,RANK() OVER (PARTITION BY personid ORDER BY MAX(effectivedate) DESC) AS RANK
            ...
            group by personid, benefitelection, benefitcoverageid, benefitsubclass, benefitplanid, personbeneelectionpid, coverageamount) pbemfsa on pbemfsa.personid = pbe.personid and pbemfsa.rank = 1  
...
left join   (select personid, benefitelection, benefitcoverageid, benefitsubclass, benefitplanid, personbeneelectionpid, coverageamount, max(effectivedate) as effectivedate, max(enddate) as enddate,RANK() OVER (PARTITION BY personid ORDER BY MAX(effectivedate) DESC) AS RANK
            ...
            group by personid, benefitelection, benefitcoverageid, benefitsubclass, benefitplanid, personbeneelectionpid, coverageamount) pbedfsa on pbedfsa.personid = pbe.personid and pbedfsa.rank = 1  
