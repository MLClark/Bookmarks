...
--,pbe.coverageamount
--,pbe.monthlyamount
--,parking.benefitsubclass as parking
--,flex.benefitsubclass as flex
,case when pbe.benefitsubclass in (parking.benefitsubclass) then pocM.employeerate
      when pbe.benefitsubclass in (flex.benefitsubclass)    then pocA.employeerate
...
LEFT JOIN personbenoptioncostl pocA
 ...
 AND poca.costsby = 'A'
 AND poca.employeerate > 0
 
LEFT JOIN personbenoptioncostl pocM
  ON pocm.personid              = pbe.personid
 AND pocm.personbeneelectionpid = pbe.personbeneelectionpid
 AND pocm.costsby = 'M'
 AND pocm.employeerate > 0
