...
,null as bene_rel
,null as bene_pct
,case when pe.effectivedate >= elu.lastupdatets and (greatest(pbeac.effectivedate,pbecie.effectivedate,pbehi.effectivedate)) >= elu.lastupdatets then 'New Enrollment' 
...
,to_char(greatest(pbeac.effectivedate,pbecie.effectivedate,pbehi.effectivedate),'mm/dd/yyyy')::char(10) as edoc
...
,pbeac.monthlyamount as ae_prem_amt    
--,ppdac.etv_amount as ae_prem_amt    
...
,case when pbeac.benefitelection = 'T' then to_char(pbeac.effectivedate,'mm/dd/yyyy')
      when pe.emplstatus = 'T' then to_char(pbeac.effectivedate,'mm/dd/yyyy')
...
,case when pbecie.benefitsubclass = '1W' then 'Critical Illness' 
      when pbecis.benefitsubclass = '1S' then 'Critical Illness' 
      when pbecic.benefitsubclass = '1C' then 'Critical Illness' 
      else null end ::varchar(30) as ci_plan_type    
,tier.cicvgtier ::varchar(30) as ci_insured_option 
...
,case when pbecie.benefitelection = 'T' then to_char(pbecie.effectivedate,'mm/dd/yyyy')
      when pe.emplstatus = 'T' then to_char(pbecie.effectivedate,'mm/dd/yyyy')
...
,case when pbehi.benefitelection = 'T' then to_char(pbehi.effectivedate,'mm/dd/yyyy')
      when pe.emplstatus = 'T' then to_char(pbehi.effectivedate,'mm/dd/yyyy')
...
-- select * from person_phone_contacts where personid = '2678';
