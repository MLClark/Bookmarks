...
----- 5/22/20 - use mailing address when available otherwise use residential

,case when pam.addresstype = 'Mail' then pam.streetaddress else pa.streetaddress end ::char(30) as addr1
,case when pam.addresstype = 'Mail' then pam.streetaddress2 else pa.streetaddress2 end ::char(30) as addr2
,' '                    ::char(30) as fill02
,case when pam.addresstype = 'Mail' then pam.city else pa.city end ::char(30) as city
,case when pam.addresstype = 'Mail' then pam.stateprovincecode else pa.stateprovincecode end ::char(2) as state
,case when pam.addresstype = 'Mail' then pam.postalcode else pa.postalcode end ::char(5) as zip
,' '                    ::char(4) as zip_plus
,case when pam.addresstype = 'Mail' and pam.countrycode = 'US' then 'USA' 
      when pa.addresstype = 'Res'   and pa.countrycode  = 'US' then 'USA' end ::char(3) as country

,case when pam.addresstype = 'Mail' and pam.createts::date <> pam.effectivedate and date_part('year',pam.effectivedate) = date_part('year',current_date)
      then to_char(pam.effectivedate,'yyyymmdd')
      when pa.addresstype = 'Res' and pa.createts::date <> pa.effectivedate and date_part('year',pa.effectivedate) = date_part('year',current_date)
      then to_char(pa.effectivedate,'yyyymmdd')  else ' ' end ::char(8) as addr_chg_date
...
---- 2020-03-25 CSD-19627 – Change to send DISABS for all leave codes and only send DISABL when an employee is termed from leave.  Per discussion with Transamerica
,case when pe.empleventdetcode in ('STP','STF','ULSTP','ULSTF','LTP','LTF','ULLTP','ULLTF') then 'DISABS'
      when pe.empleventdetcode in ('Dis','RDis') then 'DISABL'
...
---- 2020-03-25 CSD-19627 – Change to send DISABS for all leave codes and only send DISABL when an employee is termed from leave.  Per discussion with Transamerica
,case when pe.empleventdetcode in ('STP','STF','ULSTP','ULSTF','LTP','LTF','ULLTP','ULLTF') then to_char(pe.effectivedate,'yyyymmdd')
      when pe.empleventdetcode in ('Dis','RDis') then to_char(pe.effectivedate,'yyyymmdd')
...
exists (select 1 
         ...
         where pa2.personid = pi.personid
           and pa2.addresstype = 'Mail' 
           and current_date      between pa2.effectivedate and pa2.enddate
           and current_timestamp between pa2.createts      and pa2.endts
           and (pa2.effectivedate >= elu.lastupdatets::DATE 
            or (pa2.createts > elu.lastupdatets and pa2.effectivedate < coalesce(elu.lastupdatets, '2017-01-01')) ))   
or
exists (select 1 
