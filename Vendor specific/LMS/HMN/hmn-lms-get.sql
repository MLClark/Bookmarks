...
--		ed.emplstatus,
		case
			when ed.emplstatus = 'T' then to_char(coalesce(pe.paythroughdate,pe.effectivedate), 'YYYYMMdd')
			else ' '
		end::char(8) as TermDate,  -- position 619-626 - 10/14/2020 change to use paythrough date. This change required adding terms to the file.
		coalesce(left(jd.jobexperience, 2), ' ')::char(2) as JobExperience,
		coalesce(left(jd.jobcode,5), ' ')::char(5) as jobcode, -- length issues
		coalesce(to_char(pp.effectivedate, 'YYYYMMdd'), ' ')::char(8) as position_date,
...
left join edi.ediemployee eemp  on eemp.personid = ed.personid
...
left join (select personid, positionid, scheduledhours, schedulefrequency, max(effectivedate) as effectivedate, RANK() OVER (PARTITION BY personid ORDER BY max(effectivedate) DESC) AS RANK
             from pers_pos where effectivedate < enddate and current_timestamp between createts and endts and current_date >= effectivedate
            group by personid, positionid, scheduledhours, schedulefrequency) pp on pp.personid = pe.personid and pp.rank = 1   
--- por ---
left join (select positionid, organizationid, rank() over (partition by positionid order by max(effectivedate) desc) as rank 
             from pos_org_rel where posorgreltype = 'Member' and effectivedate < enddate and current_timestamp between createts and endts and current_date >= effectivedate
            group by positionid, organizationid) por on por.positionid = pp.positionid and por.rank = 1
--- ocDept ---
left join (select organizationid, organizationdesc, orgcode, rank() over (partition by organizationid order by max(effectivedate) desc) as rank 
             from organization_code where effectivedate < enddate and current_timestamp between createts and endts and current_date >= effectivedate
            group by organizationid, organizationdesc, orgcode) ocDept on ocDept.organizationid = por.organizationid and ocDept.rank = 1
--- pos ---
left join (select positionid, topositionid, rank() over (partition by topositionid order by max(effectivedate) desc) as rank
             from pos_pos where effectivedate < enddate and current_timestamp between createts and endts and current_date >= effectivedate
            group by positionid, topositionid) pos on pos.topositionid = pp.positionid and pos.rank = 1            
            
                      
left join edi.etl_employment_data edDept on edDept.positionid = pos.positionid
...
--- orel ---
left join (select organizationid, memberoforgid, rank() over (partition by organizationid order by max(effectivedate) desc) as rank 
             from org_rel where orgreltype = 'Management' and effectivedate < enddate and current_timestamp between createts and endts and current_date >= effectivedate
            group by organizationid, memberoforgid) orel on orel.organizationid = por.organizationid and orel.rank = 1
--- ocDiv ---
left join (select organizationid, organizationdesc, rank() over (partition by organizationid order by max(effectivedate) desc) as rank
             from organization_code where effectivedate < enddate and current_timestamp between createts and endts and current_date >= effectivedate
            group by organizationid, organizationdesc) ocDiv on ocDiv.organizationid = orel.memberoforgid and ocDiv.rank = 1
--- porb ---
left join (select positionid, organizationid, rank() over (partition by positionid order by max(effectivedate) desc) as rank
             from pos_org_rel where posorgreltype = 'Budget' and effectivedate < enddate and current_timestamp between createts and endts and current_date >= effectivedate
            group by positionid, organizationid) porb on porb.positionid = ed.positionid and porb.rank = 1
--- ocBudget ---            
left join (select organizationid, orgcode, rank() over (partition by organizationid order by max(effectivedate) desc) as rank
             from organization_code where effectivedate < enddate and current_timestamp between createts and endts and current_date >= effectivedate
            group by organizationid, orgcode) ocBudget on ocBudget.organizationid = porb.organizationid and ocBudget.rank = 1
--- pj ---
left join (select positionid, jobid, rank() over (partition by positionid order by max(effectivedate) desc) as rank
             from position_job where effectivedate < enddate and current_timestamp between createts and endts
            group by positionid, jobid) pj on pj.positionid = pp.positionid and pj.rank = 1  
--- jd ---            
left join (select jobid, federaljobcodeid, jobexperience, jobcode, rank() over (partition by jobid order by max(effectivedate) desc) as rank
             from job_desc where effectivedate < enddate and current_timestamp between createts and endts and current_date >= effectivedate
            group by jobid, federaljobcodeid, jobexperience, jobcode) jd on jd.jobid = pj.jobid and jd.rank = 1                     
...
left join (select personid, phoneno, phonecontacttype, effectivedate, enddate, createts, endts, rank() over (partition by personid order by effectivedate , createts ) as rank
             from person_phone_contacts where (current_date between effectivedate and enddate or (effectivedate > current_date and enddate > effectivedate)) and current_timestamp between createts and endts  
              and phonecontacttype = 'Work') ppc on ppc.personid = pi.personid and ppc.rank = 1   
...
left join  edi.edi_last_update elu on elu.feedid = 'HMN_PS_LMSGet'	
	--- 10/14/2020 termed individuals should be placed on the file and remain indefinitely. 
--where pe.emplstatus <> 'T' --and pe.personid = '68850'
     where case when pe.emplstatus = 'T' then pe.effectivedate >= elu.lastupdatets end
        or pe.emplstatus not in ('T')
