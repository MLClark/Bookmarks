select distinct
 pe.personid
,'24 on leave EES' ::varchar(40) as QSOURCE
-------------------------------------------------------------------------------------------------
,pe.emplstatus as emplstatus -- needed for kettle
,pe.emplhiredate as rawemplhiredate -- needed for kettle
...
,pe.emplstatus ::char(1) as employee_status --21
...
---- << For these 6 fields: include EEs not in Woodridge Union (Salary Grade Description) 
,sg.grade
,sg.salarygradedesc
,lc.locationcode
,pu.frequencycode
...
,case when sg.grade = '1' then ' ' 
      when sg.grade <> '1' and lc.locationcode in ('7YZ','7ZK','7VK') then '00003' 
      when sg.grade <> '1' and lc.locationcode not in ('7YZ','7ZK','7VK') then '00002' end ::char(5) as STD1account --61
...
,case when sg.grade  = '1' then ' '
      when sg.grade <> '1' and pc.frequencycode = 'A' and pd.flsacode = 'N' then '201' 
      when sg.grade <> '1' and pc.frequencycode = 'A' and sg.grade in ('6') then '202'
      when sg.grade <> '1' and pe.emplclass = 'F' and pc.frequencycode = 'H' and sg.grade not in ('6') and lc.locationcode in ('7B9') then '203'
      when sg.grade <> '1' and pc.frequencycode = 'H' and sg.grade not in ('6') and lc.locationcode in ('7C7') then '204'
      when sg.grade <> '1' and sg.grade not in ('6') and lc.locationcode in ('7DE') then '205'
      else ' ' end ::char(3) as STD1planid --62
      
,case when sg.grade = '1' then ' ' else to_char(pe.empllasthiredate,'yyyymmdd') end ::char(8) as std1_effectivedate --63 not determined in kettle
,case when sg.grade = '1' and pe.emplstatus in ('R','T') then to_char(pe.effectivedate,'yyyymmdd') else ' ' end ::char(8) as std1_term_date --64 not determined in kettle
...
,case when lc.locationcode in ('7YZ','7ZK','7VK') then '00003' 
      when lc.locationcode not in ('7YZ','7ZK','7VK') then '00002' end ::char(5) as FMLAAccount --117
...
left join  (select personid, compamount, increaseamount, compevent, frequencycode, earningscode, max(createts) as createts, MAX(effectivedate) AS effectivedate, MAX(enddate) AS enddate, 
...
-- select * from person_locations where personid in ('19756','19757','19758','19761')
...
*/
where   pe.emplstatus in ('L','P') 
    and pe.emplclass in ('F','P')
    and pp.scheduledhours > 30 
