select distinct
 pe.personid
,'1208 ACTIVE EES' ::varchar(40) as QSOURCE
-------------------------------------------------------------------------------------------------
...
,case when sg.grade = '1' then ' ' 
      when sg.grade in ('6')  and pe.emplstatus in ('R','T') then to_char(pe.effectivedate,'yyyymmdd') else ' ' end ::char(8) as std1_term_date --64 not determined in kettle
