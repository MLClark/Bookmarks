select distinct
 pi.personid 
,'ACTIVE EE FUTURE DATED BENEFIT' ::VARCHAR(20) AS SOURCESEQ
,0 as sort_seq 
...
,case when pc.frequencycode = 'H' then to_char(((pc.compamount * pp.scheduledhours) * 24)*100, 'FM000000000000000 ')
      else to_char(pc.compamount, 'FM0000000000000000 ') end ::char(16) as basic_salary_amount -- assume decimal zero fill
...
left join (select pbe.personid,bcd.benefitcoveragedesc,bcd.benefitcoverageid       
 ...
 where pbe.benefitsubclass in ('11')
   and pbe.benefitelection in ('E')
   and pbe.selectedoption = 'Y'
   group by 1,2,3
...
left join person_bene_election pbestd
 ...
 and pbestd.effectivedate >= CURRENT_DATE
...
left join person_bene_election pbeltd
 ...
 and pbeltd.effectivedate >= current_date
...
left join person_bene_election pbebeevlife
 ...
 and pbebeevlife.effectivedate >= current_date
...
left join person_bene_election pbebSPvlife
 ...
 and pbebSPvlife.effectivedate >= current_date
...
left join person_bene_election pbebDPvlife
 ...
 and pbebDPvlife.effectivedate >= current_date  
...
select distinct
 pi.personid 
,'ACTIVE DEP SPOUSE FUTURE DATED BENEFIT' ::VARCHAR(20) AS SOURCESEQ 
...
select distinct
 PI.PERSONID
,'ACTIVE DEP CHILDREN FUTURE DATED BENEFIT' ::VARCHAR(20) AS SOURCESEQ 
,case when pdr.dependentrelationship = 'SP' then 2 else 3 end as sort_seq    
-- DEMOGRAPHICS SEGMENT 1 - 325
