...
,bcd.benefitcoveragedesc,bcd.benefitcoverageid  
...
,case when pc.frequencycode = 'H' then to_char((pc.compamount * 2080) * 100, 'FM0000000000000000 ')
      else to_char(pc.compamount * 100, 'FM0000000000000000 ') end as basic_salary_amount -- assume decimal zero fill
...
left join (select pbe.personid,bcd.benefitcoveragedesc,bcd.benefitcoverageid       
 ...
 where pbe.benefitsubclass in ('11')-- only time need to join on bcd is for dental 
   ...
   and pbe.selectedoption = 'Y'
   group by 1,2,3
   ) bcd on bcd.personid = pbedent.personid 
...
where pi.identitytype = 'SSN'
  ...
  and pe.emplstatus <> 'T'
  --and bcd.benefitcoveragedesc is not null
  and pi.personid in ('314', '139')