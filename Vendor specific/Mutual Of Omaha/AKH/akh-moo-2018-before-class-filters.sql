...
,to_char(pbe_max.effectivedate, 'YYYYMMDD')::char(8) as sub_group_eff_date
...
,to_char(pbe_max.effectivedate, 'YYYYMMDD')::char(8) as class_eff_date -- position 317 date
...
,case when coalesce(pbestaffstd.benefitsubclass,pbeerstd.benefitsubclass) in ('30','32') then 'S' else ' ' end ::char(1) as product_category_S
,case when coalesce(pbestaffstd.benefitsubclass,pbeerstd.benefitsubclass) in ('30','32') then to_char(coalesce(pbestaffstd.effectivedate,pbeerstd.effectivedate),'YYYYMMDD') else ' ' end ::char(8) as pc_effective_date_s
,case when coalesce(pbestaffstd.benefitsubclass,pbeerstd.benefitsubclass) in ('30','32') and coalesce(pbestaffstd.benefitelection,pbeerstd.benefitelection) = 'T'  then 'TM' 
      when coalesce(pbestaffstd.benefitsubclass,pbeerstd.benefitsubclass) in ('30','32') and coalesce(pbestaffstd.benefitelection,pbeerstd.benefitelection) = 'E'  then 'EN' else '  ' end ::char(2) as elig_event_s
,case when coalesce(pbestaffstd.benefitsubclass,pbeerstd.benefitsubclass) in ('30','32') then 'STS00NCSAL' else ' ' end ::char(10) as plan_id_s
,case when coalesce(pbestaffstd.benefitsubclass,pbeerstd.benefitsubclass) in ('30','32') then 'C' else ' ' end ::char(1) as emp_only_s
