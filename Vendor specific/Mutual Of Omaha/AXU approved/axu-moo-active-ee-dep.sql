...
left join 
     (select personid, max(effectivedate) as effectivedate
       ...
       where current_date between effectivedate and enddate
         ...
         and benefitsubclass in ('2Z','2X''2C','2S','13')
...
where pi.identitytype = 'SSN'
  ...
  and (((pdr.dependentrelationship in ('DP','SP','NA') and pbe.benefitsubclass in ('2Z','2S')) or (pdr.dependentrelationship in ('DP','SP','NA') and pbe.benefitsubclass in ('13')))
  ...
  ;
  select * from person_bene_election where benefitsubclass in ('2Z','2S') and personid = '803';
