-- Materialized View: public.cognos_pers_pos_history_mv

-- DROP MATERIALIZED VIEW public.cognos_pers_pos_history_mv;

--CREATE MATERIALIZED VIEW public.cognos_pers_pos_history_mv AS 
 SELECT pp.personid,
    pi.identity as pspid,
    pu.payunitxid,
    pp.effectivedate,
    pp.enddate,
    pp.createts,
   ...
   FROM pers_pos pp
     left join person_identity pi on pi.personid = pp.personid and current_timestamp between pi.createts and pi.endts and pi.identitytype = 'PSPID'
     left join person_payroll ppay on ppay.personid = pi.personid and current_date between ppay.effectivedate AND ppay.enddate and current_timestamp between ppay.createts AND ppay.endts 
     left join pay_unit pu  on pu.payunitid = ppay.payunitid and current_timestamp between pu.createts and pu.endts 
     ...
     LEFT JOIN pos_pos ppo ON pp.positionid = ppo.topositionid AND "overlaps"(GREATEST(pp.effectivedate, pd.effectivedate)::timestamp with time zone, LEAST(pp.enddate, pd.enddate)::timestamp with time zone, ppo.effectivedate::timestamp with time zone, ppo.enddate::timestamp with time zone) AND ppo.effectivedate <= ppo.enddate AND now() >= ppo.createts AND now() <= ppo.endts
  ...
  WHERE pp.enddate > pp.effectivedate AND now() >= pp.createts AND now() <= pp.endts
    and pp.personid in ('34','60','146','147','132','214','22','112','49')
    order by 1,3
  
  /*
WITH DATA;

ALTER TABLE public.cognos_pers_pos_history_mv
  OWNER TO postgres;
GRANT ALL ON TABLE public.cognos_pers_pos_history_mv TO postgres;
GRANT SELECT, UPDATE, INSERT, DELETE ON TABLE public.cognos_pers_pos_history_mv TO read_write;
GRANT SELECT ON TABLE public.cognos_pers_pos_history_mv TO read_only;

-- Index: public.ix_cognos_pers_pos_history_mv

-- DROP INDEX public.ix_cognos_pers_pos_history_mv;

CREATE INDEX ix_cognos_pers_pos_history_mv
  ON public.cognos_pers_pos_history_mv
  USING btree
  (personid COLLATE pg_catalog."default", effectivedate);
*/
