...
,case when pdr.dependentrelationship in ('SP','DP') and pmse.maritalstatus = 'D' then to_char(pmse.effectivedate,'mm/dd/yyyy') 
      when pdr.dependentrelationship in ('S','D','C') and pvd.birthdate::date <= current_date::date - interval '26 year' then to_char(pvd.birthdate::date + interval '26 years','mm/dd/yyyy')
      when pe.emplstatus = 'T' then to_char(pbe.effectivedate,'mm/dd/yyyy') 
      else ' ' end ::char(10) AS termdate
,case when pdr.dependentrelationship in ('SP','DP') and pmse.maritalstatus = 'D' then 'C'
      when pe.emplstatus = 'T' then 'T'
     -- when pdr.dependentrelationship in ('S','D','C') and pvd.birthdate::date <= current_date::date - interval '26 years' then 'A'
      else ' ' end ::char(1) AS termCode
,' ' ::char(8) AS unapplied
