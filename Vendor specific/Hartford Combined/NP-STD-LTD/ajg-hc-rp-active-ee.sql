select distinct
 pi.personid
,'174 Active EE' ::varchar(30) as sourceseq
--------------------------------------------------------------
...
,case when pbe.benefitsubclass in ('30','3Y') then to_char(pbestd.effectivedate,'YYYYMMDD') else null end ::char(8) as nst_cov_effdate
,case when pbe.benefitsubclass in ('30','3Y') and pbe.benefitelection = 'T' then to_char(pbestd.effectivedate,'YYYYMMDD')
      when pbe.benefitsubclass in ('30','3Y') and pe.emplstatus = 'T' then to_char(pe.effectivedate,'YYYYMMDD')
...
,case when pbe.benefitsubclass in ('30','3Y') and pbe.benefitplanid = '288' then 'Core'
      when pbe.benefitsubclass in ('30','3Y') and pbe.benefitplanid = '291' then '60'
      when pbe.benefitsubclass in ('30','3Y') and pbe.benefitplanid = '295' then 'Buyup'
...
4 - for AJG15-16"
*/
,case when pbe.benefitsubclass in ('30','3Y') and pu.payunitxid in ('00','01') then '1'
      when pbe.benefitsubclass in ('30','3Y') and pu.payunitxid in ('05','06') then '2'
      when pbe.benefitsubclass in ('30','3Y') and pu.payunitxid in ( '20' )    then '3'
      when pbe.benefitsubclass in ('30','3Y') and pu.payunitxid in ('15','16') then '4' else null end ::char(1) as nst_ee_group_id
...
JOIN person_bene_election pbe 
 ...
 AND pbe.benefitsubclass IN ( '1W','13','30','3Y','31')
 AND pbe.selectedoption = 'Y'
 and pbe.benefitelection in ('T','E')
 and pbe.personid in (select distinct personid from person_bene_election where current_timestamp between createts and endts 
                         and benefitsubclass in ( '1W','13','30','3Y','31') and benefitelection = 'E' and selectedoption = 'Y')  
--------------------- 
----- 10KCritNT -----
---------------------
