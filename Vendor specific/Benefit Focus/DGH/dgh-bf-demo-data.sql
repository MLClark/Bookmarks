...
,'Location' ::varchar(50) as cust_cat_type2  --BB54
,lc.locationcode
,case when lc.locationcode = '7B4' then 'TECH - GBU (MAC)'
      ...
      */
      ...
      when lc.locationcode = '7YZ' then 'TECH - CERTECH WOODRIDGE (MAC)' else ' ' END ::varchar(50) as cust_cat_value2 --BC55
      
,'Union Status' ::varchar(50)  as cust_cat_type3 --BD56
,case when sg.grade in ('6') then 'NON UNION' else 'UNION' end::varchar(20) as cust_cat_value3 --BE57

,'Employment Class' ::varchar(50)  as cust_cat_type4 --BF58
/*
        1/25/19 modified emplclass - added seasonal 
*/
,case when pe.emplclass = 'F' then 'Full Time' 
      when pe.emplclass = 'T' then 'Seasonal'
      when pe.emplclass = 'P' then 'Part Time' end ::varchar(50) as cust_cat_value4      --BG59 

,'Company Name' ::varchar(50)  as cust_cat_type5 --BG60
,pu.employertaxid
,case when pu.employertaxid = '94-3146877' then 'Morgan Advanced Ceramics Inc.' 
      when pu.employertaxid = '52-2133955' then 'Certech Inc.' end ::varchar(50) as cust_cat_value5 --BH61

,'STD CLASSIFICATION' ::varchar(20) as cust_cat_type6  --NEW 5/15/19 BJ62

,case when pc.frequencycode like 'H%' then 'HOURLY'
      when pc.frequencycode like 'A%' then 'SALARY' else pc.frequencycode end ::varchar(20) as cust_cat_value6 --NEW 5/15/19 BK63
...
,to_char(pc.compamount,'999999999D00') ::char(13) as salary   --AJ36 as salary   --AJ36
,case when pc.frequencycode like 'H%' then 'HOURLY'
      when pc.frequencycode like 'A%' then 'ANNUAL' else pc.frequencycode end ::varchar(20) as earn_unit_measure --AK37
,'BIWEEKLY' ::varchar(25) as pay_freq --AL38
,to_char(pc.effectivedate,'yyyymmdd') ::varchar(8) as earn_amt_eff_dt   --AM 39 - comp effectivedate 
...
,case when lc.locationcode = '7B4' then 'TECH - GBU (MAC)'
      ...
      */
      when lc.locationcode = '7VK' then 'TECH - CERTECH TWINSBURG (MAC)'
      when lc.locationcode = '7XK' then 'TECH - CERTECH WILKESBARRE (MAC)'
      when lc.locationcode = '7YZ' then 'TECH - CERTECH WOODRIDGE (MAC)' else ' ' END ::varchar(50) as cust_cat_value2cust_cat_value2
...
left join  (select personid, scheduledhours, schedulefrequency, positionid, MAX(effectivedate) AS effectivedate, MAX(enddate) AS enddate,RANK() OVER (PARTITION BY personid ORDER BY MAX(effectivedate) DESC) AS RANK
              from pers_pos  LEFT JOIN edi.edi_last_update elu ON elu.feedid = 'DGH_Benefitfocus_Demographic_Export' AND current_timestamp BETWEEN createts AND endts and effectivedate <= elu.lastupdatets::DATE
             GROUP BY personid, scheduledhours, schedulefrequency, positionid ) as pp on pp.personid = pe.personid and pp.rank = 1
...
/*
        Selection Criteria: All Active employees, including on Leave
        Terminated Employees from their Termination Date and for 60 days after their Termination Date
*/ 
  
where pi.identitytype = 'SSN'
  and current_timestamp between pi.createts and pi.endts
  and pe.emplstatus IN ('R','T')
  and ((  (pe.emplstatus in ('A', 'L') and pe.emplclass = 'F' and pe.employmenttype = 'P'  and pp.scheduledhours > 30 and current_date between pe.effectivedate and pe.enddate and current_timestamp between pe.createts and pe.endts) 
   or (pe.emplstatus in ('T', 'R', 'D') and current_date between pe.effectivedate and pe.enddate and current_timestamp between pe.createts and pe.endts and current_date < pe.effectivedate + interval '60 days')
   or (pe.emplevent like 'FullPart%' and current_date between pe.effectivedate and pe.enddate and current_timestamp between pe.createts and pe.endts and current_date < pe.effectivedate + interval '60 days')))
