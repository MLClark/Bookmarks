...
left join ( select ppd.personid, sum(ppd.etv_amount) as etv_amount
             ...
             group by ppd.personid) lastyr on lastyr.personid = pi.personid and ((xppd.positiontitle in ('Agent')) or (xppd.positiontitle ilike ('%Financial Advisor%')) or (xppd.positiontitle ilike '%Acct Rep%' ))
...
left join ( select ppd.personid, sum(ppd.etv_amount) as etv_amount
             ...
             group by ppd.personid) curyr on curyr.personid = pi.personid and ((xppd.positiontitle ilike ('%Financial Advisor%')) or (xppd.positiontitle ilike '%Acct Rep%' ))
