...
,case when xppd.positiontitle in ('Financial Advisor','Senior Financial Advisor') and pc.compamount = 0 then to_char(comm.commission,'99999999D99')
      ...
      when pc.frequencycode = 'H' then to_char(pc.compamount * 2080 ,'99999999D99') end ::char(14) as Annsalary	                   
 ---------------------------------------------------------------------------------------------------------------	                   
...
left join ( select personid, SUM(ETV_AMOUNT) commission
             ...
             where CHECK_DATE >= current_date - interval '1 year'
               ...
               group by 1) comm on comm.personid = pe.personid --and xppd.positiontitle in ('Agent')    
