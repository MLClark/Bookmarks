...
,ppay.effectivedate as pay_unit_effectivedate
,ppay.rank
,pu.payunitxid
,lu.key1
,lu.value1
...
----- 3/20 - send pay unit effectivedate where there is a change to the pay unit

,case when pud.lastupdatets::date <= ppay.effectivedate::date and ppay.rank > 1 then to_char(ppay.effectivedate,'yyyymmdd') else null end ::char(8) as cust_cat_eff_date --AY51 ---date that any category change becomes effective
...
--- 3/24 new look back date - never gets updated starting point for pay unit change date
left join edi.edi_last_update pud on pud.feedid = 'BAC_BF_pay_unit_start_date'
...
--- 3/24 changed to use ranking
left join (select personid, compamount, frequencycode, createts, effectivedate, rank() over (partition by personid order by max(createts) desc) as rank
             from person_compensation where earningscode <> 'BenBase' and current_date between effectivedate and enddate and current_timestamp between createts and endts 
            group by personid, compamount, frequencycode, createts, effectivedate)   pc on pc.personid = pi.personid and pc.rank = 1  
...
(select ppay.personid, ppay.payunitid, ppay.effectivedate, rank() over (partition by personid order by effectivedate asc) as rank
   from person_payroll ppay where ppay.effectivedate < ppay.enddate and current_timestamp between ppay.createts and ppay.endts) ppay on ppay.personid = pi.personid   
...
where pi.identitytype = 'SSN'
  ...
  ---- 3/24 added leave to emplstatus
  and pe.emplstatus in ('L','A')-- and pe.emplpermanency <> 'T'
...
left JOIN person_net_contacts pnch 
 ...
 AND current_date between pnch.effectivedate and pnch.enddate
 AND current_timestamp between pnch.createts and pnch.enddate   
--- 3/24 changed to use ranking to get latest effectivedate
left join 
(select ppay.personid, ppay.payunitid, ppay.effectivedate, rank() over (partition by personid order by effectivedate asc) as rank
   from person_payroll ppay where ppay.effectivedate < ppay.enddate and current_timestamp between ppay.createts and ppay.endts) ppay on ppay.personid = pi.personid
