-- Materialized View: public.cognos_payment_taxesus_by_check_date_mv

-- DROP MATERIALIZED VIEW public.cognos_payment_taxesus_by_check_date_mv;
 ...
 SELECT ppds.individual_key,
   ...
   FROM pspay_payment_detail ppds
     ...
     LEFT JOIN ( SELECT taxmappaycodetoetvid.ttype_tax_code,
          ...
          WHERE taxmappaycodetoetvid.taxid <> 1) tmap
           ON tmap.etv_id::text = ppds.etv_id::text AND tmap.ttype_tax_code = ppds.ttype_tax_code

     LEFT JOIN tax t ON t.taxid = tmap.taxid  
