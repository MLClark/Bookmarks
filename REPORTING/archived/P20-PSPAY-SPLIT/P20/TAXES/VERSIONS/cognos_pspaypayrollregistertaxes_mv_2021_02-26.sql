--CREATE MATERIALIZED VIEW public.cognos_pspaypayrollregistertaxes_mv AS 
 SELECT 
   ...
   FROM pspay_payment_header ph
     LEFT JOIN company_parameters cp ON cp.companyparametername = 'PInt'::bpchar
     ---- removed pgdate join
     ...
     JOIN person_tax_elections pte on pte.personid = pd.personid and current_date between pte.effectivedate and pte.enddate and current_timestamp between pte.createts and pte.endts and pte.taxid = tla.taxid
