...
JOIN (SELECT pd.personid,pd.paymentheaderid,t.taxid,pd.paycode as pd_paycode,payc.paycode as pc_paycode,pd.units,pd.amount,pd.subject_wages,pd.units_ytd,pd.amount_ytd,pd.subject_wages_ytd, payc.paycodeshortdesc,t.taxiddesc
        ...
        JOIN payroll.pay_codes payc ON payc.paycode::text = pd.paycode::text and current_date between payc.effectivedate and payc.enddate and current_timestamp between payc.createts and payc.endts
...
ALTER TABLE payroll.cognos_pspaypayrollregistertaxes_mv
  OWNER TO postgres;
GRANT ALL ON TABLE payroll.cognos_pspaypayrollregistertaxes_mv TO postgres;
GRANT SELECT, UPDATE, INSERT, DELETE ON TABLE payroll.cognos_pspaypayrollregistertaxes_mv TO read_write;
GRANT SELECT ON TABLE payroll.cognos_pspaypayrollregistertaxes_mv TO read_only;
