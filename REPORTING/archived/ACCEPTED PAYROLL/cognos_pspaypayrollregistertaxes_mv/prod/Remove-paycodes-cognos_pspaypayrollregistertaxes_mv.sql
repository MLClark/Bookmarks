 ...
 SELECT ph.personid,
    ...
    /*
        CASE
            ...
            ELSE NULL::character varying END AS etv_id,
   ...
   FROM pspay_payment_header ph
     ...
     JOIN pspay_payment_detail pd ON pd.paymentheaderid = ph.paymentheaderid AND pd.etv_id::text ~~ 'T%'::text AND NOT (pd.ttype_tax_code = ANY (ARRAY[' '::bpchar, '000000'::bpchar])) 
      AND (pd.etv_amount_ytd <> 0::numeric OR pd.etv_taxable_wage_ytd <> 0::numeric OR pd.etv_amount <> 0::numeric OR pd.etv_taxable_wage <> 0::numeric)
     ...
     LEFT JOIN company_tax_setup cts ON (pu.employertaxid::character varying::text = ANY (cts.feinlist::text[])) AND now() >= cts.createts AND now() <= cts.endts AND rpgd.asofdate >= cts.effectivedate AND rpgd.asofdate <= cts.enddate AND (cts.taxid = tla.taxid OR cts.taxid = trp.taxider)
     JOIN payroll.taxmappaycodetoetvid tmap ON tmap.etv_id::text = pd.etv_id::text AND tmap.ttype_tax_code = pd.ttype_tax_code
     JOIN payroll.pay_codes payc ON payc.paycode::text = tmap.new_paycode AND pte.taxid = tmap.taxid
     JOIN company_parameters cp ON cp.companyparametername = 'PInt'::bpchar AND cp.companyparametervalue = 'P20'
