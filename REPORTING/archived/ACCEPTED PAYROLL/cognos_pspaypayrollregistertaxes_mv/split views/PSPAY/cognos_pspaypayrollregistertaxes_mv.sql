...
/*
DROP VIEW IF EXISTS public.cognos_pspaypayrollregistertaxes;
...
*/
 SELECT ph.personid,
   ...
   FROM pspay_payment_header ph
     ...
     JOIN cognos_pspaypayroll_persontaxelectionsbyyear_mv pte ON pte.personid = ph.personid AND date_part('year'::text, rpgd.asofdate) = pte.tlayear AND pte.etvid_lookups @> ARRAY[pd.etv_id] AND pd.ttype_tax_code = pte.ttype_tax_code
     ...
     LEFT JOIN company_parameters cp ON cp.companyparametername = 'PInt'::bpchar AND cp.companyparametervalue::text = 'PSPAY'::text
