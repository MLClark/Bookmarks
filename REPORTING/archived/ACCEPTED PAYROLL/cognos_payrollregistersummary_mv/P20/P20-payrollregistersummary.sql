CREATE MATERIALIZED VIEW payroll.cognos_payrollregistersummary_mv AS 
 SELECT DISTINCT ph.personid,
    ((pn.fname::text || ' '::text) || COALESCE(pn.mname::text || ' '::text, ''::text) || pn.lname::text || ' ' || COALESCE(pn.title::text || ' '::text, ''::text)) AS employeename,
    ((pn.lname::text || ' '::text || pn.fname::text) || ' '::text || COALESCE(pn.mname, ''::character VARYING)::text || ' ' || COALESCE(pn.title::text || ' '::text, ''::text)) AS employeenamelastfirst, 
     
    ph.paymentheaderid,
    pip.identity AS employeeid,              -----ph.individual_key AS employeeid,
    ph.check_date::character varying AS check_date,
    pu.payunitdesc AS group_key,             -----ph.group_key,
    psp.periodstartdate,                     -----ph.period_begin_date::character varying AS periodstartdate,
    psp.periodenddate,                       -----ph.period_end_date::character varying AS periodenddate,
    ...
    pgd.periodpaydate,
    pp.pspaypayrollid,
    ph.sequencenumber AS payment_number,   -----ph.payment_number,
    ...
    phc.is_voided,
    phc.is_reissued,
    case when pp.pspaypayrollstatusid = '4' then 'Y' else 'N' end as payrollfinal            ----ph.payrollfinal
   FROM payroll.payment_header ph
     JOIN company_parameters cp ON cp.companyparametername = 'PInt'::bpchar and cp.companyparametervalue::text = 'P20'::text     ----- new limit to p20 clients   
     JOIN paymentheaderclassifications phc ON phc.paymentheaderid = ph.paymentheaderid
     
     LEFT JOIN person_identity pip ON pip.personid = ph.personid AND now() >= pip.createts AND now() <= pip.endts AND pip.identitytype = 'PSPID'  ----new payroll does not have pspid id 
     JOIN pay_schedule_period psp ON psp.payscheduleperiodid = ph.payscheduleperiodid -----new payroll.ph does not have periodstart and periodend dates
     ...
     JOIN pay_unit pu ON pgd.payunitid = pu.payunitid AND current_timestamp between pu.createts and pu.endts  ----- added current_timestamp 
...
ALTER TABLE payroll.cognos_payrollregistersummary_mv
  OWNER TO postgres;
GRANT ALL ON TABLE payroll.cognos_payrollregistersummary_mv TO postgres;
GRANT SELECT ON TABLE payroll.cognos_payrollregistersummary_mv TO read_only;
GRANT SELECT, UPDATE, INSERT, DELETE ON TABLE payroll.cognos_payrollregistersummary_mv TO read_write;
