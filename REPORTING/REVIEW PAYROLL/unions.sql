 ...
 SELECT st1.payscheduleperiodid,
   ...
   FROM ( SELECT ppp.payscheduleperiodid,
          ...
          GROUP BY ppp.payscheduleperiodid, ppp.personid, pp.pspaypayrollid, pes.personearnsetuppid, st.paycode, st.rate, pc.paycodeshortdesc, ppp.paymenttype, (COALESCE(ppp.sequencenumber, '~'::character varying))) st1
     JOIN ( SELECT ppp.payscheduleperiodid,
           ...
           FROM payroll.personperiodpayments ppp
             ...
             JOIN pspay_payroll pp ON ps.pspaypayrollid = pp.pspaypayrollid
             LEFT JOIN person_earning_setup pes ON pes.personid = st.personid AND pes.etvid::text = st.paycode::text AND psp.periodpaydate >= pes.effectivedate AND psp.periodpaydate <= pes.enddate AND now() >= pes.createts AND now() <= pes.endts
          GROUP BY ppp.payscheduleperiodid, ppp.personid, pp.pspaypayrollid, pes.personearnsetuppid, st.paycode, st.rate, pc.paycodeshortdesc, ppp.paymenttype, (COALESCE(ppp.sequencenumber, '~'::character varying))) st2 ON st2.personid = st1.personid AND st2.etv_id::text = st1.etv_id::text AND st2.payscheduleperiodid = st2.payscheduleperiodid
  GROUP BY st1.payscheduleperiodid, st1.personid, st1.paymentheaderid, st1.pspaypayrollid, st1.personearnsetuppid, st1.etv_id, st1.etvname, st1.paymenttype, st2."coalesce"
          ...
          WHERE (p2.payscheduleperiodid IN ( SELECT pay_schedule_period.payscheduleperiodid
                  ...
                  WHERE date_part('year'::text, pay_schedule_period.periodpaydate) >= date_part('year'::text, ( SELECT min(pay_schedule_period_1.periodpaydate) AS min
                          ...
                          WHERE (pay_schedule_period_1.payscheduleperiodid IN ( SELECT staged_transaction.payscheduleperiodid
                                   FROM payroll.staged_transaction))))))
          GROUP BY p2.payscheduleperiodid, p2.personid, p2.paymentheaderid, p2.pspaypayrollid, p2.personearnsetuppid, p2.etv_id, p2.etvname, p2.amount, p2.hours, p2.rate, p2.net_pay, p2.gross_pay, p2.ytd_hrs, p2.ytd_amount, p2.ytd_wage, p2.paymenttype, p2.sequencenumber) p21
     JOIN ( SELECT p2.payscheduleperiodid,
          ...
          WHERE (p2.payscheduleperiodid IN ( SELECT pay_schedule_period.payscheduleperiodid
                  ...
                  WHERE date_part('year'::text, pay_schedule_period.periodpaydate) >= date_part('year'::text, ( SELECT min(pay_schedule_period_1.periodpaydate) AS min
                          ...
                          WHERE (pay_schedule_period_1.payscheduleperiodid IN ( SELECT staged_transaction.payscheduleperiodid
                                   FROM payroll.staged_transaction))))))
          GROUP BY p2.payscheduleperiodid, p2.personid, p2.paymentheaderid, p2.pspaypayrollid, p2.personearnsetuppid, p2.etv_id, p2.etvname, p2.amount, p2.hours, p2.rate, p2.net_pay, p2.gross_pay, p2.ytd_hrs, p2.ytd_amount, p2.ytd_wage, p2.paymenttype, p2.sequencenumber, p2.paymentdetailid) p22 ON p22.personid = p21.personid AND p22.payscheduleperiodid = p21.payscheduleperiodid AND p22.paymentheaderid = p21.paymentheaderid AND p22.pspaypayrollid = p21.pspaypayrollid AND p22.etv_id::text = p21.etv_id::text AND p22.rank = 1
  GROUP BY p21.personid, p21.payscheduleperiodid, p21.paymentheaderid, p21.pspaypayrollid, p21.personearnsetuppid, p21.etv_id, p21.etvname, p21.paymenttype, p21."coalesce", p22.ytd_hrs, p22.ytd_amount, p22.ytd_wage;
