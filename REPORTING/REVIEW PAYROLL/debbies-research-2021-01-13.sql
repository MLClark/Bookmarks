/*
select etv_id, sum(amount) as amount, sum(net_pay) as net_pay, sum(gross_pay) as gross_pay,
 sum(ytd_amount) as ytd_amount, sum(ytd_wage) as ytd_wage
 from payroll.cognos_proposedpayrollregisterdeductions
where payscheduleperiodid in (1443,1419,1491)
group by etv_id;

select * from payroll.cognos_proposedpayrollregisterdeductions where payscheduleperiodid in (1443,1419,1491) and etv_id in ('VB3','VB5')
order by personid, etv_id;



select * from edi.etl_personal_info where personid = '1019';


select * from payroll.staged_transaction where paycode = 'V39-ER';

select * from pay_unit;
select * from pay_schedule_period where periodpaydate > '2021-01-01' order by periodpaydate limit 30;

select * from payroll.staged_transaction where payscheduleperiodid in (1443,1419,1491)
and paycode in ('VB5','VB3')
order by personid, paycode

*/
--  - view from S drive as a with
with prevdeds as ( SELECT st1.payscheduleperiodid,
    st1.personid,
    st1.paymentheaderid,
    st1.pspaypayrollid,
  ...
  WHERE ppp.net_amount <> 0::numeric --and ppp.personid = '179' and st.paycode = 'VAF'
  GROUP BY ppp.payscheduleperiodid, ppp.personid, pp.pspaypayrollid, st.paycode, pc.paycodedesc, pc.paycodeshortdesc, pc.paycodetypeid, ppp.paymenttype, st.taxid, pds.persondedsetuppid, pgs.persongarnishmentsetuppid, gt.garntypedesc, ppp.sequencenumber, st.amount
  ) st1
...
JOIN ( SELECT ppp.payscheduleperiodid,
    ...
    pst.subject_wages_ytd AS ytd_wage,
        CASE
            WHEN pc.paycodetypeid = 6 THEN 'T'::text
            ELSE 'F'::text
        END AS isemployer,
   ...
   LEFT JOIN garnishment_type gt ON pgs.garnishmenttype = gt.garntype
  WHERE ppp.net_amount <> 0::numeric --and ppp.personid = '179' and st.paycode = 'VAF'
  GROUP BY ppp.payscheduleperiodid, ppp.personid, pp.pspaypayrollid, st.paycode, pc.paycodedesc, pc.paycodeshortdesc, pc.paycodetypeid, ppp.paymenttype, st.taxid, pds.persondedsetuppid, pgs.persongarnishmentsetuppid, gt.garntypedesc, ppp.sequencenumber, st.amount, st.units, st.rate, ppp.net_amount, ppp.gross_amount, pst.amount_ytd, pst.subject_wages_ytd
  ) st2
  ON st2.personid = st1.personid AND st2.etv_id::text = st1.etv_id::text AND st1.payscheduleperiodid = st2.payscheduleperiodid
  GROUP BY st1.payscheduleperiodid, st1.personid, st1.paymentheaderid, st1.pspaypayrollid, st1.persondedsetuppid, st1.etv_id, st1.etvname, st1.paymenttype, st1.sequencenumber, st1.persongarnishmentsetuppid, st1.isemployer, st2.net_pay, st2.gross_pay, st2.ytd_amount, st2.ytd_wage

UNION

SELECT  
    prd1.payscheduleperiodid,
    prd1.personid,
    prd1.paymentheaderid,
    prd1.pspaypayrollid,
    prd1.persondedsetuppid,
    prd1.persongarnishmentsetuppid,
    prd1.etv_id,
    prd1.etvname,
    
    sum(COALESCE(prd1.amount, 0::numeric)::numeric(18,2)) AS amount,
    sum(COALESCE(prd1.hours, 0::numeric)::numeric(18,6)) AS hours,
    sum(COALESCE(prd1.rate, 0::numeric)::numeric(18,6)) AS rate,
    COALESCE(prd2.net_pay, 0::numeric) AS net_pay,
    COALESCE(prd2.gross_pay, 0::numeric) AS gross_pay,
    COALESCE(prd2.ytd_amount, 0::numeric) AS ytd_amount,
    COALESCE(prd2.ytd_wage, 0::numeric) AS ytd_wage,
    
    prd1.isemployer,
    prd1.paymenttype,
    prd1.sequencenumber 

FROM ( SELECT payrollregisterdeductions.payscheduleperiodid,
...
group by payrollregisterdeductions.payscheduleperiodid,
    ...
    payrollregisterdeductions.paymenttype,
    payrollregisterdeductions.sequencenumber   
    ) prd2 on prd2.personid = prd1.personid and prd2.etv_id = prd1.etv_id  AND prd2.payscheduleperiodid = prd1.payscheduleperiodid and prd2.paymentheaderid = prd1.paymentheaderid and prd1.pspaypayrollid = prd2.pspaypayrollid
     GROUP BY prd1.payscheduleperiodid, prd1.personid, prd1.paymentheaderid, prd1.pspaypayrollid, prd1.persondedsetuppid, prd1.etv_id, prd1.etvname, prd1.paymenttype, prd1.sequencenumber, prd1.persongarnishmentsetuppid, prd1.isemployer, prd2.net_pay, prd2.gross_pay, prd2.ytd_amount, prd2.ytd_wage
)

select SUM(ytd_amount)
from prevdeds 
where payscheduleperiodid in (1419) and etv_id in ('VB5')
--group by etv_id

 ;   
SELECT sum(ytd_amount) from payroll.cognos_proposedpayrollregisterdeductions where payscheduleperiodid = '1419'   and etv_id in ('VB5');  
