...
--CREATE OR REPLACE VIEW payroll.cognos_proposedpayrollregistersummary AS 
 SELECT DISTINCT ppp.personid,
    ...
    pgd.payunitid,
        ...
        END AS is_livecheck,
        CASE
            WHEN ppp.paymenttype = 7 THEN 'T'::text
            ELSE 'F'::text
        END AS is_immediatecheck,
        CASE
            WHEN ppp.paymenttype = 2 THEN 'T'::text
            ELSE 'F'::text
        END AS is_adjustment,
        CASE
            WHEN ppp.paymenttype = 4 THEN 'T'::text
            ELSE 'F'::text
        END AS is_postpayment,
    NULL::text AS is_voided,
    NULL::text AS is_reissued,
    ppp.paymenttype,
    pt.processingorder,
...
UNION
 SELECT payrollregistersummary.personid,
    ...
    payrollregistersummary.payunitid,
        ...
        END AS is_livecheck,
        CASE
            WHEN payrollregistersummary.is_immediatecheck THEN 'T'::text
            ELSE 'F'::text
        END AS is_immediatecheck,
        CASE
            WHEN payrollregistersummary.is_adjustment THEN 'T'::text
            ELSE 'F'::text
        END AS is_adjustment,
        CASE
            WHEN payrollregistersummary.is_postpayment THEN 'T'::text
            ELSE 'F'::text
        END AS is_postpayment,
        CASE
            WHEN payrollregistersummary.is_voided THEN 'T'::text
            ELSE 'F'::text
        END AS is_voided,
        CASE
            WHEN payrollregistersummary.is_reissued THEN 'T'::text
            ELSE 'F'::text
        END AS is_reissued,
