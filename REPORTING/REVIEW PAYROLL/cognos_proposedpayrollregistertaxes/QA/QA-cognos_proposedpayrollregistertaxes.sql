...
--CREATE OR REPLACE VIEW payroll.cognos_proposedpayrollregistertaxes AS 
 SELECT ppp.payscheduleperiodid,
    ...
    COALESCE(st.units_ytd, 0.00) AS ytd_hrs,
  ...
  GROUP BY ppp.payscheduleperiodid, ppp.personid, pp.pspaypayrollid, st.paycode, pc.paycodeshortdesc, pc.paycodetypeid, ppp.paymenttype, st.taxid, t.taxiddesc, pu.employertaxid, trp.taxider, pte.persontaxelectionpid, ppp.sequencenumber, st.amount_ytd, st.subject_wages_ytd, st.units_ytd, pt.paymenttypedesc
UNION
...
ALTER TABLE payroll.cognos_proposedpayrollregistertaxes
  OWNER TO skybotsu;
GRANT ALL ON TABLE payroll.cognos_proposedpayrollregistertaxes TO skybotsu;
