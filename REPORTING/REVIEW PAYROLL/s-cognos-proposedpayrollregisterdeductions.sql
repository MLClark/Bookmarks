...
(SELECT ppp.payscheduleperiodid,
  ...
  WHERE ppp.net_amount <> 0::numeric
   --and ppp.personid = '92' and ppp.payscheduleperiodid = 744
  GROUP BY ppp.payscheduleperiodid, ppp.personid, pp.pspaypayrollid, st.paycode, pc.paycodedesc, pc.paycodeshortdesc, pc.paycodetypeid, ppp.paymenttype, st.taxid, pds.persondedsetuppid, pgs.persongarnishmentsetuppid, gt.garntypedesc, ppp.sequencenumber, st.amount, st.paycodeaffiliationid
...
(SELECT ppp.payscheduleperiodid,
  ...
  ,st.amount_ytd, st.subject_wages_ytd
) st2    ON st2.personid = st1.personid AND st2.etv_id::text = st1.etv_id::text AND st1.payscheduleperiodid = st2.payscheduleperiodid
  GROUP BY st1.payscheduleperiodid, st1.personid, st1.paymentheaderid, st1.pspaypayrollid, st1.persondedsetuppid, st1.etv_id, st1.etvname, st1.paymenttype, st1.sequencenumber, st1.persongarnishmentsetuppid, st1.amount, st1.hours, st1.rate, st1.isemployer, st2.net_pay, st2.gross_pay, st2.ytd_amount, st2.ytd_wage, st1.paycodeaffiliationid, st1.enroll_in_catchup  
