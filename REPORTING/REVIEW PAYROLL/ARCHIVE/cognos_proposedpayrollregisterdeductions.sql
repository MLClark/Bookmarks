...
--CREATE OR REPLACE VIEW payroll.cognos_proposedpayrollregisterdeductions AS 
 SELECT st1.payscheduleperiodid,
   ...
   FROM ( SELECT ppp.payscheduleperiodid,
          ...
          GROUP BY ppp.payscheduleperiodid, ppp.personid, pp.pspaypayrollid, st.paycode, pc.paycodedesc, pc.paycodeshortdesc, pc.paycodetypeid, ppp.paymenttype, st.taxid, pds.persondedsetuppid, pgs.persongarnishmentsetuppid, gt.garntypedesc, ppp.sequencenumber, st.paycodeaffiliationid, st.rate) st1
