...
--CREATE OR REPLACE VIEW payroll.cognos_proposedpayrollregisterdeductions AS 
 SELECT st1.payscheduleperiodid,
   ...
   FROM ( SELECT ppp.payscheduleperiodid,
          ...
          GROUP BY ppp.payscheduleperiodid, ppp.personid, pp.pspaypayrollid, st.paycode, pc.paycodedesc, pc.paycodeshortdesc, pc.paycodetypeid, ppp.paymenttype, st.taxid, pds.persondedsetuppid, pgs.persongarnishmentsetuppid, gt.garntypedesc, ppp.sequencenumber, st.paycodeaffiliationid, st.rate, pt.paymenttypedesc) st1
     JOIN ( SELECT ppp.payscheduleperiodid,
           ...
           FROM payroll.personperiodpayments ppp
             ...
             JOIN ( SELECT staged_transaction.personid,
                    ...
                    rank() OVER (PARTITION BY staged_transaction.personid, staged_transaction.paycode, staged_transaction.paymenttype, staged_transaction.sequencenumber ORDER BY (max(staged_transaction.paymentseq)) DESC) AS rank
                   FROM payroll.staged_transaction
                  GROUP BY staged_transaction.personid, staged_transaction.sequencenumber, staged_transaction.payscheduleperiodid, staged_transaction.asofdate, staged_transaction.paycode, staged_transaction.units_ytd, staged_transaction.amount_ytd, staged_transaction.subject_wages_ytd, staged_transaction.paymenttype, staged_transaction.paymentseq) pst 
                  ON pst.payscheduleperiodid = st.payscheduleperiodid AND pst.personid = st.personid AND pst.asofdate = st.asofdate AND pst.paycode::text = st.paycode::text AND pst.paymentseq = st.paymentseq and pst.paymenttype = st.paymenttype AND pst.rank = 1
             JOIN payroll.pay_codes pc ON pc.paycode::text = st.paycode::text AND pc.uidisplay = 'Y'::bpchar AND 'now'::text::date >= pc.effectivedate AND 'now'::text::date <= pc.enddate AND now() >= pc.createts AND now() <= pc.endts AND (pc.paycodetypeid = ANY (ARRAY[3, 6]))
             JOIN pay_schedule_period psp ON psp.payscheduleperiodid = st.payscheduleperiodid
             JOIN pspay_payroll_pay_sch_periods ps ON ps.payscheduleperiodid = psp.payscheduleperiodid
             JOIN pspay_payroll pp ON ps.pspaypayrollid = pp.pspaypayrollid
...
--ALTER TABLE payroll.cognos_proposedpayrollregisterdeductions  OWNER TO skybotsu;
