...
CREATE MATERIALIZED VIEW public.cognos_pspaypayrollregistertaxes_winter_mv AS 
 SELECT ph.personid,
   ...
   FROM pspay_payment_header ph
     ...
     LEFT JOIN public.cognos_pspaypayroll_persontaxelectionsbyyear_mv pte ON pte.personid = ph.personid AND date_part('year'::text, rpgd.asofdate) = pte.tlayear AND pte.etvid_lookups @> ARRAY[pd.etv_id] AND pd.ttype_tax_code = pte.ttype_tax_code
