...
--CREATE OR REPLACE VIEW public.cognos_covid_credit_detail AS 
 SELECT pph.personid,
    ...
    ccd.ee_medicare_wages,
    ccd.er_social_sec_tax,
    ccd.imported
  ...
  WHERE (pph.paymentheaderid IN ( SELECT ph.paymentheaderid
          ...
          WHERE (ph.employer_id IN ( SELECT unnest(company_tax_setup_federal.feinlist) AS unnest
                  ...
                  WHERE 'now'::text::date >= company_tax_setup_federal.effectivedate AND 'now'::text::date <= company_tax_setup_federal.enddate AND now() >= company_tax_setup_federal.createts AND now() <= company_tax_setup_federal.endts AND (company_tax_setup_federal.legislationid = ANY (ARRAY[1, 5])) AND company_tax_setup_federal.taxid = 1))))
