...
count(*)
, case 
      when pc.frequencycode = 'A' then sum(pc.compamount)
      --when pc.frequencycode = 'H' then (pc.compamount * 2080 * pe.emplfulltimepercent)
      when pc.frequencycode = 'H' then sum((pc.compamount * 2080 ))
      end                              as toalsalryforcontrib                 -- col AR

,sum(hw.ytdhours) as totalhoursworked
,pc.frequencycode
