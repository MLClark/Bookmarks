...
,case when dedamt.group_key not in ('BAC05','BAC15') then dedamt.vb6_amount
      else null end     as profit_sharing          ----- col AR 
----------------------------------------------------------------
...
,case when dedamt.group_key in ('BAC05','BAC15') then dedamt.vb6_amount
      else null end     as profit_sharing_cur      ----- col AX     
----------------------------------------------------------------  
...
where pi.identitytype = 'SSN'
  ...
  and pe.emplstatus in ('T', 'R', 'D') and pe.effectivedate < pe.enddate and pe.enddate >= '2199-12-30' and current_timestamp between pe.createts and pe.endts and current_date - interval '45 days' <= pe.effectivedate     
