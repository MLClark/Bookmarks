select distinct
 pi.personid
,elu.lastupdatets
,oc_dept.orgcode ::char(4) as orgcode
,date_part('year',age(pv.birthdate))
,dedamt.total_401kded  as total_ded
,loans.total_loan_amount as total_loans
,pu.payunitxid
...
,replace(pa.streetaddress,',',' ') as addr1 --J
,replace(pa.streetaddress2,',',' ') as addr2 --K
,pa.city as city --L
,pa.stateprovincecode --M
,pa.postalcode --N
,to_char(pv.birthdate,'mm/dd/yyyy') as dob --O
,to_char(pe.emplhiredate,'mm/dd/yyyy') as doh --P
-----Changed 6/7/2021: Per client / vendor meeting - don't provide a rehire date if no prior terminations IE  ('3033','2664','3038')
,case when ((pe.emplhiredate <> pe.empllasthiredate) and rehired.effectivedate is not null) then to_char(pe.empllasthiredate,'mm/dd/yyyy') else ' ' end as ldoh --Q

-----Changed 5/13/2021: Vendor would like Termination Date populated whenever there is a Rehire Date
,case when pe.emplstatus in ('R','T') then to_char(pe.effectivedate,'mm/dd/yyyy')
      when rehired.personid is not null then to_char(rehired.effectivedate,'mm/dd/yyyy')
      else ' ' end as termdate --R
...
,case when loans.etv_id = 'V65' then loans.referencenumber end ::varchar(25) as loan_number1 --AJ
,case when loans.etv_id = 'V65' then loans.v65_amount end as loan_amount1 --AK
,case when loans.etv_id = 'V73' then loans.referencenumber end ::varchar(25) as loan_number2 --AL
,case when loans.etv_id = 'V73' then loans.v73_amount end as loan_amount1 --AM
,case when loans.etv_id = 'V31' then loans.referencenumber end ::varchar(25) as loan_number3 --AN
...
,sum(x.v25_amount + x.v27_amount + x.vb1_amount + x.vb2_amount + x.vb3_amount + x.vb4_amount + x.vb5_amount + x.vcg_amount + x.vch_amount) as total_401kded
...
group by personid, check_date, etv_id, referencenumber, rank ) loans on loans.personid = pe.personid  and loans.key_rank = 1
        
where pi.identitytype = 'SSN'
  and current_timestamp between pi.createts and pi.endts

  and (pe.emplpermanency = 'P' --- exclude temporary ee's
    or pe.emplpermanency <> 'P' and loans.total_loan_amount <> 0 
    or pe.emplpermanency <> 'P' and dedamt.total_401kded  <> 0 ) 
  and (date_part('year',age(pv.birthdate)) >= 21.0 
    or date_part('year',age(pv.birthdate)) <= 21.0 and loans.total_loan_amount <> 0 
    or date_part('year',age(pv.birthdate)) <= 21.0 and dedamt.total_401kded  <> 0 )
  and ((pe.emplstatus in ('L','A','P') and (pp.schedulefrequency = 'B' and pp.scheduledhours > 35.0)) --include biweekly scheduled hours > 35
    or (pe.emplstatus in ('L','A','P') and (pp.schedulefrequency = 'B' and pp.scheduledhours <= 35.0 and dedamt.total_401kded  <> 0 ))
    or (pe.emplstatus in ('L','A','P') and (pp.schedulefrequency = 'B' and pp.scheduledhours <= 35.0 and loans.total_loan_amount <> 0 ))
