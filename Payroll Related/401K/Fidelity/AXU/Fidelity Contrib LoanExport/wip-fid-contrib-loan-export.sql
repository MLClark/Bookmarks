...
from
    ...
    left join (select * from edi.lookup where lookupid in (select lookupid from edi.lookup_schema where lookupname = 'FidelityLoanETVIdMap')) letvid on 1=1 		

-- loan 1  V65

	left join (

		select distinct  individual_key, personid,  sum(etv_amount) as loan_amount
    ...
    and ppd.etv_id  in  (
				...
				where	ls.lookupname = 'FidelityLoanETVIdMap'
					...
					and value1 is not null)
						group by ppd.individual_key, ppd.personid
...
-------------------------------------------------------------------------------------------------
-- c_ee  employee contribution to 401K         contribution_employee          ('VB1', 'VB2')
-------------------------------------------------------------------------------------------------
	left join (select distinct  personid, etv_id, sum(etv_amount) as contribution_employee
...
--                               on elu.feedid = 'Fidelity_ContributionLoan' and '2019-09-15'::DATE <= ppay.statusdate
   ...
   -- and ppd.etv_id  in  ('VB1', 'VB2')
		group by ppd.personid, ppd.etv_id
		order by ppd.personid

) c_ee on c_ee.personid = pe.personid and c_ee.etv_id in (letvid.value5,letvid.value6)


-- employer contribution to Roth	contribution_roth	('VB3', 'VB4')  V25?
left join (select distinct  personid, etv_id, sum(etv_amount) as contribution_roth
...
--                                 on elu.feedid = 'Fidelity_ContributionLoan' and '2019-09-15'::DATE <= ppay.statusdate
    ...
    --and ppd.etv_id  in  ('VB3', 'VB4') -- Do we need to add V25?
		...
		order by ppd.personid
			...
			) c_rth on c_rth.personid = pe.personid and c_rth.etv_id in (letvid.value7,letvid.value8)
		
		
-- employer contribution to 401K	contribution_employer	= 'VB5'
	left join (select distinct  personid, etv_id, sum(etv_amount) as contribution_employer
...
--                                 on elu.feedid = 'Fidelity_ContributionLoan' and '2019-09-15'::DATE <= ppay.statusdate
    ...
    --and ppd.etv_id  = 'VB5'
		group by ppd.personid, ppd.etv_id
		order by ppd.personid
			) c_er on c_er.personid = pe.personid and c_er.etv_id in (letvid.value9)

----------------------------------			
----- AFTER TAX V25 FOR AXU ------
----------------------------------			
left join (select distinct  personid, etv_id, sum(etv_amount) as contribution_aftertax 
  ...
  --  and ppd.etv_id  in  ('V25')  
		...
		order by ppd.personid
			...
			) c_aftax on c_aftax.personid = pe.personid and c_aftax.etv_id in (letvid.value4)
...
order by pe.personid, referencenumber
