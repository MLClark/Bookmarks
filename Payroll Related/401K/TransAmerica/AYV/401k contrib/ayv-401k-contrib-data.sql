...
,'46 CONTRIBUTION'  ::varchar(50) as qsource
...
,cast(coalesce(mtdloan.taxable_wage,0.00) as dec(10,2)) as LoanRepayments
,cast(coalesce(grossytd.taxable_wage,0.00) as dec(10,2)) as CompensationYTD
,cast(0.00 as dec(10,2)) as ExcludedCompensationYTD
,cast(coalesce(pdx.workedhours,0) as dec(4,0)) as HoursWorkedYTD
...
join pspay_payment_detail ppd
 ...
 and ppd.etv_id in ('V65','VB1','VB2','VB3','VB4','VB5')
...
LEFT JOIN (SELECT personid
            ...
            WHERE etv_id in  ('V65')    
              and check_date = ?::DATE   
              AND group_key in ('AYV00','AYV05')    
              --AND date_part('year', check_date) = date_part('year',?::DATE)
              --AND date_part('month', check_date) = date_part('month',?::DATE)
         GROUP BY personid
          ) mtdloan
  ON mtdloan.personid = ppd.personid 
...
LEFT join (select pd.personid
   ...
   left join pspay_etv_operators peo 
         ...
         and peo.opcode = 'A'
       where extract(year from pd.check_date) = date_part('year',?::DATE)
       group by pd.personid, extract(year from pd.check_date))pdx 
  on pdx.personid = pI.personid 
------------------------------
------ 401K Deductions -------
------------------------------
---- dgh jh 401k uses mulitple loans 
left join 
(select 
 x.personid
,x.check_date
...
join pspay_payment_detail ppd 
 ...
 and ppd.group_key in ('AYV00','AYV05')

where pi.identitytype = 'SSN'::bpchar 
  AND current_timestamp between pi.createts and pi.endts and ppd.etv_id  in  ('V65','VB1','VB2','VB3','VB4','VB5')) x
  group by 1,2 having sum(x.vb1_amount + x.vb2_amount + x.vb3_amount + x.vb4_amount + x.vb5_amount + x.v65_amount) <> 0) dedamt on dedamt.personid = pi.personid 
...
,pe.emplclass
,'02' ::char(2) as recordtype
,'108 ACTIVE ELIGIBLE EE'   ::varchar(50) as qsource
,lc.locationcode ::char(5) as location -- use business unit (org structure)
,' ' ::char(5) as sub_company_nbr
,' ' ::char(5) as group_code
,pi.identity ::char(9) as ssn
...
,'0' ::char(1) as hdr_deposit_method
,' ' ::char(26) as hdr_ach_number
,'Validate' ::char(8) as hdr_mode --- validate for test blank for prod
...
where pi.identitytype = 'SSN'
  and current_timestamp between pi.createts and pi.endts
  and pe.emplstatus not in ('T','R') and pe.emplclass = 'F' 
...
,pe.emplclass
,'02' ::char(2) as recordtype
,'TERMED ELIGIBLE EE for up to 1 year'   ::varchar(50) as qsource
,lc.locationcode ::char(5) as location -- use business unit (org structure)
,' ' ::char(5) as sub_company_nbr
,' ' ::char(5) as group_code
,pi.identity ::char(9) as ssn
...
where pi.identitytype = 'SSN'
  and current_timestamp between pi.createts and pi.endts
  and pe.emplstatus in ('T','R') and pe.emplclass = 'F' 
  and current_date - interval '1 year' <= pe.effectivedate
  and pu.payunitxid in ('00','05')
  and pe.personid not in 
  (select personid from pspay_payment_detail ppd where ppd.check_date = ?::DATE and ppd.group_key in ('AYV00','AYV05')  and ppd.etv_id in ('V65','VB1','VB2','VB3','VB4','VB5') group by 1)  
