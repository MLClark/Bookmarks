...
,'097408' as retire_planid
, pi2.identity::char(9) as SSNO
, 'A' ::char(1) as data_group_A
, upper(pn.lname||', '||pn.fname||COALESCE(pn.mname,''))  	AS  empName
, '  ' ::char(2) AS status
...
, lc.locationid :: char(04) as loc_code4
, EPI.locationid ::char(10) as loc_code10
...
, CASE WHEN eed.empllasthiredate > eed.emplhiredate THEN to_char(eed.empllasthiredate,'YYYYMMDD') ELSE null  END AS rehiredate
, ' ' ::char(04) as vestingovrpct
, ' ' ::char(01) as vestingovrpctind
, ' ' ::char(01) AS payrollfreq
, ' ' ::char(34) AS fill1
, ' ' ::char(10) as payroll_code_one
