...
, '"'||piE.identity||'"' ::char(11) as employeenbr
...
, ptcu.PretaxCUpContribAmt             as PretaxCUpContribAmt           -- col AY should have the PreTax EE Catchup Amount (per pay) 
...
, rdf.rothDeferral                     as rothDeferral                  -- col BE roth deferrel

, ' '::char(1) as OtherContribAmt 
, rcu.RothCUContribAmt                 as RothCUContribAmt              -- col BG Roth Catch Up Contribution Amount
, ' '::char(1) as OtherContribAmt
, ' '::char(1) as LoanNbrForLoanRepayment1

, loan.loan1payment                    as loan1payment                  -- col BJ loan repayment
...
left JOIN (SELECT personid
                 ,sum(etv_amount) as rothDeferral
             FROM pspay_payment_detail            
             -- Hours include Reg,OT,DT,PTO,Jury,Bereavement - exclude reg shift and OT shift
            WHERE etv_id in  ('VB3')           
              AND date_part('year', check_date) = date_part('year',now())
  ...
  ON rdf.personid = pi.personid
  

--- RothCUContribAmt  

left JOIN (SELECT personid
                 ,sum(etv_amount) as RothCUContribAmt  
             FROM pspay_payment_detail            
             -- Hours include Reg,OT,DT,PTO,Jury,Bereavement - exclude reg shift and OT shift
            WHERE etv_id in  ('VB4')           
              AND date_part('year', check_date) = date_part('year',now())
  ...
  ON rcu.personid = pi.personid  
--- PretaxCUpContribAmt   
 
left JOIN (SELECT personid
                 ,sum(etv_amount) as PretaxCUpContribAmt  
             FROM pspay_payment_detail            
             -- Hours include Reg,OT,DT,PTO,Jury,Bereavement - exclude reg shift and OT shift
            WHERE etv_id in  ('VB2')           
              AND date_part('year', check_date) = date_part('year',now())
  ...
  ON ptcu.personid = pi.personid    
  
--- loan1payment

left JOIN (SELECT personid
                 ,sum(etv_amount) as loan1payment  
             FROM pspay_payment_detail            
             -- Hours include Reg,OT,DT,PTO,Jury,Bereavement - exclude reg shift and OT shift
            WHERE etv_id in  ('V65')           
              AND date_part('year', check_date) = date_part('year',now())
