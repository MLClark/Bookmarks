...
--YTD Values
,cast(coalesce(grossytd.YTD_taxable_wage,0.00) as decimal(10,2)) as planCompensationYTD
,cast(coalesce(grossytd.YTD_hours,0) as decimal(10,2)) as HoursWorkedYTD
...
where ppd.paymentheaderid in 
 ...
 group by personid, periodpaydate, check_date having sum(x.vdo_amount + x.vdp_amount + x.vb6_amount + x.loan1_amount + x.loan2_amount) <> 0) dedamt on dedamt.personid = pi.personid
...
*/            
---------------------------------------------------
------ grosscur Current 401K Taxable Wages --------
---------------------------------------------------
left join 
(select wh.personid, wh.check_date, sum(wh.units) as hours, sum(wh.amount) as taxable_wage
   from    
       (select 
              ppd.personid
             ,ppd.amount
             ...
             where pspaypayrollid in (select pspaypayrollid from pspay_payroll ppay join edi.edi_last_update elu on elu.feedid = 'KD0_LincolnFinancial_403B_Contributions' and coalesce( ?::timestamp, elu.lastupdatets)  <= ppay.statusdate where ppay.pspaypayrollstatusid = 4 ))) 
...
) grosscur on grosscur.personid = pi.personid and grosscur.check_date = dedamt.check_date  
-------------------------------------  -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
) grossytd on grossytd.personid = pi.personid and grossytd.check_date = dedamt.check_date    
...
(select
ppd.personid
,case when ppd.paycode = 'VDO' AND date_part('year', ppd.check_date) = date_part('year',current_date) then amount else 0 end as vdo_amount  -- 403b EE deductions
,case when ppd.paycode = 'VDP' AND date_part('year', ppd.check_date) = date_part('year',current_date) then amount else 0 end as vdp_amount  -- 403b EE Catchup
,case when ppd.paycode = 'VB6' AND date_part('year', ppd.check_date) = date_part('year',current_date) then amount else 0 end as vb6_amount  -- Employer Match
,case when ppd.paycode = 'VAT' AND date_part('year', ppd.check_date) = date_part('year',current_date) then amount else 0 end as loan1_amount  -- Loan 1
,case when ppd.paycode = 'VAU' AND date_part('year', ppd.check_date) = date_part('year',current_date) then amount else 0 end as loan2_amount  -- Loan 2
        ...
        (select paymentheaderid from pspay_payment_header where payscheduleperiodid in 
                (select payscheduleperiodid from pspay_payroll_pay_sch_periods where pspaypayrollid in 
                        (select pspaypayrollid from pspay_payroll ppay 
					       ...
					       AND date_part('year', ppay.statusdate) = date_part('year',current_timestamp) )))) x 			       
...
left join (select psp.payunitid, max(periodpaydate) as periodpaydate
            ...
            where psp.payrolltypeid = 1 and psp.processfinaldate::date is not null and psp.periodpaydate >= coalesce( ?::timestamp, elu.lastupdatets)  group by 1) psp on psp.payunitid = pu.payunitid
------------------------------------------------------
