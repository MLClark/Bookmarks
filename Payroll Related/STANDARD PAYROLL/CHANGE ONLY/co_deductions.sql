...
,pbe.enddate
,case when bpd.benefitcalctype = 'P' or bpd.benefitcalctype = 'PM' then '%' else '' end as calccode
,bs.benefitsubclassdesc
,CASE WHEN pbcst.benefitelection = 'W'::bpchar THEN 0::double precision
      WHEN pbe.source = 'CB'::bpchar THEN pbcst.employeerate + pbcst.employercost::double precision
      ELSE pbcst.employeerate END AS employeerate
...
LEFT JOIN personbenoptioncostl pbcst 
       ON pbe.personid = pbcst.personid 
      AND pbe.personbeneelectionpid = pbcst.personbeneelectionpid 
      AND pbcst.costsby = 'P'::text
      
where bpd.edtcode <> '' 
  and ((pbe.enddate > NOW() and pbe.effectivedate <= pbe.enddate) 
   OR (pbe.enddate >= current_date - interval '30 days' and pbe.effectivedate <= pbe.enddate))
  and current_timestamp between pbe.createts and pbe.endts 
  and pi2.personid = '899'
	
	order by pbe.enddate
