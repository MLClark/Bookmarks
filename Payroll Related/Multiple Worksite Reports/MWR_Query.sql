...
select 
  nm.companyid
, replace(ci.companyidentity,'-','')::char(9) as fein
, CASE WHEN nm.companydba IS NOT NULL THEN nm.companydba::char(35) ELSE nm.companyname::char(35) END AS tradename
...
from (SELECT DISTINCT pl.locationid, pph.individual_key 
      ...
      WHERE pph.check_date BETWEEN DATES.prevQTR_begin_dt and DATES.prevQTR_end_dt
        AND DATES.m1_12_dt between  pph.period_begin_date  and  pph.period_end_date )x
