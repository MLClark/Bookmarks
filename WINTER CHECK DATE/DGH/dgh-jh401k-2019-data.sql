...
*/ 
left join (select ppd.personid,sum(etype_hours) AS hours 
...
*/                
left join (select ppd.personid,sum(etv_amount) AS taxable_amount
             from pay_schedule_period psp 
             join pspay_payment_header pph
               on pph.check_date = psp.periodpaydate
              and pph.payscheduleperiodid = psp.payscheduleperiodid              
...
*/  
left join (select ppd.personid, sum(ppd.etv_amount) AS taxable_amount
...
where pi.identitytype = 'SSN'
  ...
  (select distinct pe.personid 
    ...
    where pe.emplstatus in ('R','T')
      and (pe.effectivedate >= elu.lastupdatets::DATE or (pe.createts > elu.lastupdatets and pe.effectivedate < coalesce(elu.lastupdatets, '2017-01-01'))))))
