...
, sum(pph.state_gross_pay ) as qtr_state_gross_pay_CD
, sum(pph.gross_pay)  as qtr_gross_pay_CD

, sum(pphS.state_gross_pay ) as qtr_state_gross_pay_PBD
, sum(pphS.gross_pay)  as qtr_gross_pay_PBD

, sum(pphB.state_gross_pay ) as qtr_state_gross_pay_BOTH
, sum(pphB.gross_pay)  as qtr_gross_pay_BOTH
...
left join pspay_payment_header pphS
  on pphS.personid = pl.personid 
 AND DATES.m1_12_dt between pphS.period_begin_date  and pphS.period_end_date
 
left join pspay_payment_header pphB
  on pphB.personid = pl.personid 
 and pphB.check_date BETWEEN DATES.prevQTR_begin_dt and DATES.prevQTR_end_dt  
 AND DATES.m1_12_dt between pphB.period_begin_date  and pphB.period_end_date
