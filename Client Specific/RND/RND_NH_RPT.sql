...
SELECT DISTINCT
 ...
 --,pi.personid as emp_personid
 --,ppoSup1.personid as sup_personid

 --, ppSup1.topositionid as employee
 --, ppSup1.positionid as supervisor
 --, pinSup1.personid as edipi_personid
 --, pinSup1_empno.personid as pi_super_personid
 ...
 ,coalesce(pos.effectivedate,ppoSupt.effectivedate,ppSup1.effectivedate) as d_positiontitle ----Position Effective Date
 ,coalesce(pinSup1.employeeid,pinSupt.employeeid,pinSup1_empno.identity)::varchar(40) AS supervisorid
 ,coalesce(pinSup1.emailaddress,pinSupt.emailaddress,pinSup1_pnc.url)::varchar(100) AS supervisorurl
 ,coalesce(pinSup1.firstname,pinSupt.firstname,pinSup1_pn.fname)::varchar(100) AS sup_firstname
 ,coalesce(pinSup1.lastname,pinSupt.lastname,pinSup1_pn.lname)::varchar(100) AS sup_lastname
 ,ppSupt.effectivedate as ppsupt
 ,coalesce(ppSup1.effectivedate,ppSupt.effectivedate)::varchar(100) AS sup_date_orig
 ,greatest(ppSup1.effectivedate, pos.effectivedate, ppoSup1.effectivedate , ppoSupt.effectivedate)::varchar(100) AS sup_date ---Manager Effective Date 
 ,greatest(por.effectivedate,por.effectivedate, pos.effectivedate, ppoSup1.effectivedate , ppoSupt.effectivedate)::varchar(100) AS d_department  ---Department Effective Date
 ,coalesce(oc.organizationdesc,oct.organizationdesc) as department
 ,coalesce(oc.orgcode,oct.orgcode) as deptnum
 ,coalesce(pl.effectivedate,plt.effectivedate) as d_location ----- Location Effective Date
...
LEFT JOIN pers_pos ppoSup1 
  --ON ppoSup1.positionid = ppSup1.topositionid
...
---- had to add join to person_identity because we have some supervisors that can't be found on edi.etl_personal_info pinSup1 
---- personid 121420 - his identitytype is SIN not SSN

  
  
left join person_identity pinSup1_empno
  on pinSup1_empno.personid = ppoSup1.personid
 and current_timestamp between pinSup1_empno.createts and pinSup1_empno.endts
 and pinSup1_empno.identitytype = 'EmpNo'

LEFT JOIN person_names pinSup1_pn 
  ON pinSup1_pn.personid = pinSup1_empno.personid 
 AND pinSup1_pn.nametype = 'Legal'::bpchar 
 AND current_date between pinSup1_pn.effectivedate AND pinSup1_pn.enddate 
 AND current_timestamp between pinSup1_pn.createts AND pinSup1_pn.endts

LEFT JOIN person_net_contacts pinSup1_pnc 
  ON pinSup1_pnc.personid = pinSup1_empno.personid 
 AND pinSup1_pnc.netcontacttype = 'WRK'::bpchar
 AND current_date between pinSup1_pnc.effectivedate AND pinSup1_pnc.enddate 
 AND current_timestamp between pinSup1_pnc.createts AND pinSup1_pnc.endts
