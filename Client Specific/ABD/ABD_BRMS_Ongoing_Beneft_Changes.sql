...
EE Waived and Termed
*/

select distinct
----#1 finds coverage effective dates for current terms 
 pi.personid
,'0' as dependentid
...
where pi.identitytype = 'SSN'
  ...
  AND pbe.effectivedate >= elu.lastupdatets
  and pbe_eff_cov.effectivedate is not null


union

select distinct
----#2 last effective coverage for not so recent terms 
 pi.personid
,'0' as dependentid
,'T' ::char(1) as transtype  
,'E' ::char(1) as record_type
...
LEFT JOIN person_bene_election pbe_eff_cov  
  ON pbe_eff_cov.personid = pI.personid
 AND pbe_eff_cov.benefitsubclass = '10'
 AND pbe_eff_cov.benefitelection ='E'
 and pbe_eff_cov.effectivedate < pbe_eff_cov.enddate
 and date_part('year',pbe_eff_cov.enddate) = DATE_PART('YEAR',CURRENT_DATE)
...
where pi.identitytype = 'SSN'
  AND current_timestamp between pi.createts and pi.endts
  AND pbe.effectivedate >= elu.lastupdatets
  and pi.personid  in ('3992')  ----- DONT COMMENT OUT - MARIA PALAMIA
