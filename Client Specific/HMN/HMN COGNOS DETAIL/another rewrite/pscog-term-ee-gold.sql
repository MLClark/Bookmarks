select distinct
 pi.personid
,'330 TERMED EES' ::CHAR(20) as sourceseq 
--,pp.positionid
--,porb.positionid 
--,pj.positionid
--,pj.jobid
...
,to_char(coalesce(TRUNC(ytdfica.fica -((ytdAb.ALL_bonus * .0620)+(ytdAb.ALL_bonus * .0145)),2),ytdfica.fica,0),'00000009d00')
      ::char(12) as f41_o25_ytd_fica_less_ytd_fica_bonus_fica_sev
,case when occc.orgcode  = '515110' then 'Y'
      when pe.emplstatus =  'T' and ytdsev.severance_pay <> 0 then 'Y' else 'N' end as f42_o30_sev_ind
...
left join (select a.*
            ...
            LEFT JOIN edi.edi_last_update elu ON elu.feedid = 'HMN_PS_CogDetGet_Export'
             ...
             and pe.emplstatus = 'T' 
           where asofdate = pe.effectivedate - interval '1 day'
             and effectivedate <= elu.lastupdatets::DATE ) as vpcpira on vpcpira.personid = pc.personid and vpcpira.earningscode = pc.earningscode
