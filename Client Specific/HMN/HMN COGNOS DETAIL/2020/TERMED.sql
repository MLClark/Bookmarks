select distinct
 pi.personid
,'331 TERMED EES' ::CHAR(20) as sourceseq 
--,pp.positionid
--,porb.positionid 
--,pj.positionid
--,pj.jobid
...
,case when pc.compevent in ('Temp') then '0' else to_char(round(coalesce(vpcpira.percent_in_range,0),4),'0d0000') end as f19_o14_pct_in_range

--,to_char(pc.effectivedate,'mm/dd/yy') as f20_o15_sal_change_date 
,case when pc.compevent in ('Temp') then ' ' else to_char(lastcompdt.effectivedate,'mm/dd/yy') end ::char(8) as f20_o15_sal_change_date 
...
,case when pc.compevent in ('Temp','Hire','Rehire','ReHire') then '0'  else to_char(lastcompdt.increaseamount,'00000009d00') end ::char(12) as f22_o19_comp_change_amount
...
,case when pc.compevent in ('Temp','Rehire','ReHire') then '0' else
      to_char(coalesce((pc.compamount - lastcompdt.increaseamount),0),'00000000d00') end ::char(12) as f24_o21_prev_annual_salary 

,case when pc.compevent in ('Temp','Hire','Rehire','ReHire') then ' ' else pufv3.ufvalue end as f25_o16_perf_review_desc
,case when pc.compevent in ('Temp','Hire','Rehire','ReHire') then ' ' 
