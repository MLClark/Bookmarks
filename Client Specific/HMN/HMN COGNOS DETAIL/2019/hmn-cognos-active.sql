select distinct
pi.personid
-- kimberly a johnson is 2 people dupe is allowed.
,'1465 ACTIVE EES' ::CHAR(20) as sourceseq
...
,pd.positiontitle ::varchar(50) as f7_position_title
...
,case when pc.compevent = 'Temp' then '0.0000' 
...
,case when pc.compevent in ('Temp') then ' ' 
      else to_char(lastcompdt.effectivedate,'mm/dd/yy') end ::char(8) as f20_o15_sal_change_date 
...
,case when pc.compevent in ('Temp') then '0' 
      else to_char(lastcompdt.increaseamount,'00000009d00') end ::char(12) as f22_o19_comp_change_amount
...
--,pc.compamount,lastcompdt.increaseamount
,case when pc.compevent in ('Temp') then '0' else to_char(coalesce((pc.compamount - lastcompdt.increaseamount),0),'00000000d00') end ::char(12)  as f24_o21_prev_annual_salary 
--,to_char(coalesce((pc.compamount - lastcompdt.increaseamount),0),'00000000d00')::char(12) as f24_o21_prev_annual_salary 
...
,case when pe.emplstatus = 'A' then 'Acti' 
      when pe.emplstatus = 'L' then 'LOA' end ::char(4) as f28_o27_cognos_status
,to_char(pe.emplservicedate,'mm/dd/yy') ::char(8) as f29_o28_adj_hire_date ---- use service date
,to_char(cast(date_part('year',age(current_date,pe.emplservicedate)) + (date_part('month',age(current_date,pe.emplservicedate)) * .10) as dec (18,2)),'00.0')
   as f30_o29_years_of_service  

,null as f31_pay_through_date  
,null as f32_o32_term_date
...
,to_char(coalesce(TRUNC(ytdfica.fica -((ytdAb.ALL_bonus * .0620)+(ytdAb.ALL_bonus * .0145)),2),ytdfica.fica,0),'00000009d00') ::char(12) as f41_o25_ytd_fica_less_ytd_fica_bonus_fica_sev

,case when occc.orgcode  = '515110' then 'Y' else 'N' end as f42_o30_sev_ind
...
left join (SELECT personid, emplstatus, emplevent,  empleventdetcode, MAX(effectivedate) AS effectivedate, MAX(enddate) AS enddate, RANK() OVER (PARTITION BY personid ORDER BY MAX(effectivedate) DESC) AS RANK
            FROM person_employment pe1  WHERE enddate > effectivedate AND current_timestamp BETWEEN createts AND endts and emplstatus = 'L' and enddate <= current_date
...
left join (select personid, max(percomppid) as percomppid
             ...
             group by 1 
             ) as maxpc on maxpc.personid = pe.personid
...
left join pers_pos pp 
 ...
 and elu.lastupdatets::DATE between pp.effectivedate and pp.enddate
...
left join personcomppercentinrange vpcpira
  on vpcpira.personid = pc.personid        
 and vpcpira.earningscode = pc.earningscode
 and current_date >= vpcpira.effectivedate  
...
,case when pp.scheduledhours = 86.67 then to_char(((ppd05.shift_dif_amt / pp.scheduledhours) * (pp.scheduledhours * 24)) + pc.compamount,'00000000d00') 
      else null end ::char(12) as f18_o13_ee_annual_salary 
...
left join ( 
  ...
  on sdrate.personid = pe.personid
----- E01 hours - need to recompute annaul salary which requires hours from e01 also need hours for calcing shift dif rate

left join (
        select personid, etype_hours, max(check_date) as check_date, RANK() OVER (PARTITION BY personid ORDER BY MAX(check_date) DESC) AS RANK
        from pspay_payment_detail 
        where etv_id = 'E01' group by 1,2) as e01 on e01.personid = pe.personid and e01.rank = 1
...
left join ( 
  ...
  on ysdrate.personid = pe.personid --ysdrate.shift_dif_amt
...
--,porb.organizationid
-- kimberly a johnson is 2 people dupe is allowed.
,'34 pc, pe, pp changes' ::CHAR(20) as sourceseq
...
--,to_char(pc.effectivedate,'mm/dd/yy') as f20_o15_sal_change_date 
,case when pc.compevent in ('Hire','Rehire','ReHire') 
      then ' ' else to_char(lastcompdt.effectivedate,'mm/dd/yy') end ::char(8) as f20_o15_sal_change_date 

,case when pc.compevent in ('Hire','Rehire','ReHire') then '0' 
      when (pc.compamount - pc.increaseamount) <> 0 
      then to_char(round((pc.increaseamount / (pc.compamount - pc.increaseamount)) * 100, 2 ),'0000.00')
      end as f21_o18_current_comp_chg_pct
...
,case when pc.compevent in ('Hire','Rehire','ReHire') then '0' else to_char(coalesce((pc.compamount - lastcompdt.increaseamount),0),'00000000d00') end ::char(12)  as f24_o21_prev_annual_salary 

,case when pc.compevent in ('Hire','Rehire','ReHire') then ' ' else pufv3.ufvalue end as f25_o16_perf_review_desc
,case when pc.compevent in ('Hire','Rehire','ReHire') then ' ' 
      when pufv2.ufvalue in ('Y','Regular','Combined') then ' ' else pufv2.ufvalue end as f26_o17_perf_review_rating
...
left join (select personid, max(percomppid) as percomppid
             from person_compensation 
             LEFT JOIN edi.edi_last_update elu ON elu.feedid = 'HMN_PS_CogDetGet_Export'
             where elu.lastupdatets::DATE between effectivedate and enddate
               and current_timestamp between createts and endts
             group by 1 
             ) as maxpc on maxpc.personid = pe.personid
...
left join person_compensation pc
  on pc.personid = maxpc.personid
 and pc.percomppid = maxpc.percomppid
 and elu.lastupdatets::DATE between pc.effectivedate and pc.enddate
...
left join ( 
  ...
  on sdrate.personid = pi.personid
...
where pi.identitytype = 'SSN'
  ...
  and pd.positiontitle <> ('Agent')
  and elu.lastupdatets::DATE between pc.effectivedate and pc.enddate
  and pc.enddate < '2199-12-30'
  --and pe.personid = '63685'
