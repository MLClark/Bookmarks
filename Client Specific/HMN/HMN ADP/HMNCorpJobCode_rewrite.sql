...
left join ( select pp.positionid, pn.fname || pn.lname as name, pn.personid
             from pers_pos pp
...
left join ( select distinct pp.positionid,ppma.personid,pn.fname || pn.lname as name 
              from pers_pos pp
...
join ( select distinct pp.positionid,ppma.personid,ppma.fname || ppma.lname as name 
              from pers_pos pp
              join pos_pos pos 
