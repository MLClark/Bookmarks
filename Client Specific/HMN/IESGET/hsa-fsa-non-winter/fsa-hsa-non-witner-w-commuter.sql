...
,case when fsa.counter > 1 and ppd.etv_id = 'VBA' then 'Combination FSA '
      when fsa.counter = 1 and ppd.etv_id = 'VBA' then 'Health FSA'
      when ppd.etv_id = 'VBB' then 'Dependent Care'
      when ppd.etv_id = 'VL1' then 'Park-P/T '
      when ppd.etv_id = 'VS1' then 'Trns-P/T '
...
,case when ppd.etv_code = 'EE' then 'Payroll Deduction'
      when ppd.etv_code = 'ER' then 'Employer Contribution' else ' ' end ::varchar(100) as contrib_desc -- based on payroll codes either ee or er contrib
...
,case when ppd.etv_id in ('VEJ','VEH','VEK') and ppd.etv_code = 'EE' then 'EE CONTRIB '||date_part('year',current_date)
      when ppd.etv_id in ('VEJ','VEH','VEK') and ppd.etv_code = 'ER' then 'ER CONTRIB '||date_part('year',current_date)
...
,case when ppd.etv_code = 'EE' and ppd.etv_id = 'VEH' then ppd.veh_amount       --- TOTAL DEDUCTION AMOUNT HSA 
      when ppd.etv_code = 'ER' and ppd.etv_id = 'VEK' then ppd.vek_amount       --- TOTAL DEDUCTION AMOUNT HSA CO
...
,case when ppd.etv_code = 'EE' and ppd.etv_id = 'VEH' then 'HSA   ' 
      when ppd.etv_code = 'ER' and ppd.etv_id = 'VEK' then 'HSA CO' 
...
where ppd.etv_id  in ('VBA','VBB','VEJ','VEK','VEH','VS1','VL1') and check_date = ?::date ) x
  group by 1,2,3,4 having sum(x.vba_amount + x.vbb_amount + x.vej_amount + x.vek_amount + x.veh_amount + x.vl1_amount + x.vs1_amount) <> 0) ppd on ppd.personid = pi.personid 

left join (select distinct ppd.personid, count(ppd.etv_id) as counter 
from pspay_payment_detail ppd where ppd.etv_id  in ('VBA','VBB','VEJ','VEK','VEH','VS1','VL1') and check_date = ?::date
     
  group by 1) fsa on fsa.personid = ppd.personid

where pi.identitytype = 'SSN'
  and current_timestamp between pi.createts and pi.endts
 -- and pi.identity = '240436853'
 
  and pi.personid = '63407'
