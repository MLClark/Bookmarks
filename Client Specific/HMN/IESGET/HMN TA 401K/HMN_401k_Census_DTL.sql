...
,case when pe.emplstatus = 'A' then to_char(pe.effectivedate,'yyyymmdd') else to_char(pe.emplservicedate,'yyyymmdd') end ::char(8) as last_hire_date 
...
,case when pe.empleventdetcode in ('STP','STF','ULSTP','ULSTF') then to_char(pe.effectivedate,'yyyymmdd') --Employment Status Effective Date ( STD Start Date)
      when pe.empleventdetcode in ('LTP','LTF','ULLTP','ULLTF') then to_char(pe.effectivedate,'yyyymmdd') --Employment Status Effective Date (LTD Start Date)
      when pe.empleventdetcode in ('ULCHD','ULFME','ULFMM','ULMLC','ULMLE') then to_char(pe.effectivedate,'yyyymmdd') --Employment Status Effective Date (Leave Start Date)
      when pe.empleventdetcode in ('SRed') then to_char(pe.effectivedate,'yyyymmdd') --Employment Status Effective Date (Termination Date)
      when pe.empleventdetcode in ('ULMIL') then to_char(pe.effectivedate,'yyyymmdd') --Employment Status Effective Date (Leave Start Date)
      when pe.empleventdetcode in ('ULWC') then to_char(pe.effectivedate,'yyyymmdd') --Employment Status Effective Date (Leave Start Date)
      when pe.empleventdetcode in ('Death') then to_char(pe.effectivedate,'yyyymmdd') --Employment Status Effective Date (Termination Date)
      when pe.emplevent in ('VolTerm') then to_char(pe.effectivedate,'yyyymmdd') --Employment Status Effective Date (Termination Date)
      when pe.emplevent in ('InvTerm') then to_char(pe.effectivedate,'yyyymmdd') --Employment Status Effective Date (Termination Date)
      when pe.emplevent in ('Retire') then to_char(pe.effectivedate,'yyyymmdd') --Employment Status Effective Date (Termination Date)
      when pe.emplevent in ('L','LOA w/o Pa') then to_char(pe.effectivedate,'yyyymmdd') --Employment Status Effective Date (Leave Start Date)
      when pe.emplevent in ('FullPart','Hire','PartFull','Rehire') then to_char(pe.empllasthiredate,'yyyymmdd') --Last Hire Date or LOA Return Date
      when pe.empleventdetcode in ('LvR','CHD','TP')then to_char(pe.empllasthiredate,'yyyymmdd') --Employment Status Effective Date (Leave Start Date)
