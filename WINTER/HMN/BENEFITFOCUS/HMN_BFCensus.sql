...
,pam.addresstype
,pa.addresstype        
        ,case when pam.streetaddress is not null then replace(pam.streetaddress,',',' ')  else replace(pa.streetaddress,',',' ') end      ::char(30) as addr1       
        ,case when pam.streetaddress is not null then replace(pam.streetaddress2,',',' ') else replace(pa.streetaddress2,',',' ') end      ::char(30) as addr2
        ,case when pam.streetaddress is not null then pam.city else pa.city  end             ::char(30) as city
        ,case when pam.streetaddress is not null then pam.stateprovincecode else pa.stateprovincecode end  ::char(2) as state
        ,case when pam.streetaddress is not null then ltrim(pam.postalcode,' ') else ltrim(pa.postalcode,' ')::char(5) end as zip
        ,case when pam.streetaddress is not null and  pam.countrycode = 'US' then 'USA' 
              when pam.streetaddress is null and      pa.countrycode  = 'US' then 'USA'  end ::char(3) as country
        ...
        ,'' ::char(1) as fill_1
        ,'' ::char(1) as fill_2
        ,replace(ppc.phoneno,'-','') ::char(10) as phone_home
        ,replace(ppcm.phoneno,'-','') ::char(10) as phone_cell
        ,'' ::char(1) as fill_3
        ,'' ::char(1) as fill_4
        ,'' ::char(1) as fill_5
        ,'' ::char(1) as fill_6
 ...
 ---------------------------------------------------------------------------------------------------------------
        ,case --when xppd.positiontitle in ('Financial Advisor','Senior Financial Advisor') and pc.compamount = 0 then to_char(comm.commission,'99999999D99')
              when pc.frequencycode = 'A' then to_char(pc.compamount,'99999999D99')
...
/*        
left join (select personid, phoneno, enddate, max(effectivedate) as effectivedate, max(createts) as endts,  RANK() OVER (PARTITION BY personid ORDER BY max(endts) DESC) AS RANK
...
where pi.identitytype = 'SSN'
  ...
  --and xppd.positiontitle not like ('%Financial Advisor%')
  --and xppd.positiontitle not like ( '%Acct Rep%' )
  and xppd.positiontitle not in ( select lkup.value1 as positiontitle
...
,'COMMISSION BASED' ::CHAR(20) AS SOURCESEQ
--,xppd.positiontitle 
...
,pa.addresstype           
        ...
        ,CASE WHEN xppd.positiontitle ilike '%Financial Advisor%' and pufv.ufvalue = 'Y' THEN to_char(pufv.effectivedate,'yyyymmdd')
              WHEN xppd.positiontitle ilike '%Financial Advisor%' and pe.emplHiredate <> pe.emplservicedate THEN to_char(pe.emplservicedate,'yyyymmdd') 
              else '' ::char(8) END as adj_hire_date
        ,case when pe.emplstatus = 'T' then to_char(pe.paythroughdate,'yyyymmdd')else ''  end ::char(8) as TermDate -- 4/19/19 - Employment Status Effective Date (Termination Date changed to paythroughdate)
...
left join ( select ppd.personid, sum(ppd.etv_amount) as etv_amount
             ...
             group by ppd.personid) lastyr on lastyr.personid = pi.personid  
             and xppd.positiontitle
                  in ( select lkup.value1 as positiontitle
                         from edi.lookup lkup
                         join edi.lookup_schema lkups
                           on lkups.lookupid = lkup.lookupid
                          and current_date between lkups.effectivedate and lkups.enddate
...
left join ( select ppd.personid, sum(ppd.etv_amount) as etv_amount
             ...
             group by ppd.personid) curyr on curyr.personid = pi.personid 
             and xppd.positiontitle
                  in ( select lkup.value1 as positiontitle
                         from edi.lookup lkup
                         join edi.lookup_schema lkups
...
,pa.addresstype   
	...
	,pe.empleventdetcode
        ...
        left join person_phone_contacts ppc
         ...
         and current_date between ppc.effectivedate and ppc.enddate
         and current_timestamp between ppc.createts and ppc.endts
---- select * from person_phone_contacts where personid = '63551' and phonecontacttype = 'Work' and current_date between effectivedate and enddate and current_timestamp between createts and endts;
        left join person_phone_contacts ppcw
          on ppcw.personid = pi.personid
         and ppcw.phonecontacttype = 'Work'
         and current_date between ppcw.effectivedate and ppcw.enddate
