...
,'ACTIVE WINTER EES' ::CHAR(20) as sourceseq
...
,case when pc.compevent in ('Temp') then '00.00' else to_char(pp.scheduledhours,'00.00') end as f12_o49_scheduled_hours
...
,case when pc.compevent in ('Temp') then '0' 
      when lastcompdt.increaseamount <> 0 then to_char(pc.effectivedate,'mm/dd/yy') end ::char(8) as f20_o15_sal_change_date 
...
,case when pc.compevent in ('TempConv','Temp','Hire','ReHire') then '0'
      when (pc.compamount - lastcompdt.increaseamount) <> 0 then coalesce(to_char(round((lastcompdt.increaseamount / (pc.compamount - lastcompdt.increaseamount)) * 100, 2 ),'0000.00'),'0')
...
,case when pc.compevent in ('TempConv','Temp','Hire','ReHire') then '0' else to_char(coalesce((pc.compamount - lastcompdt.increaseamount),0),'00000000d00') end ::char(12)  as f24_o21_prev_annual_salary       
...
,to_char(cast(date_part('year',age(elu.lastupdatets::date,pe.emplservicedate)) + (date_part('month',age(elu.lastupdatets::date,pe.emplservicedate)) * .10) as dec (18,2)),'00.0') as f30_o29_years_of_service  
...
left join personcomppercentinrange vpcpira
 ...
 and vpcpira.percent_in_range <> 0
  --- select * from personcomppercentinrange
...
--------------------------------------------------------
----- Job and fedaral job code and salary grade joins 
--------------------------------------------------------
...
---------------------------------------------------------------
----- User Defined Values - linked to job performance reviews
---------------------------------------------------------------
...
(select distinct pe.personid, pes.amount
       ...
       ,case when pp.scheduledhours = 86.67 then to_char(((ppd05.shift_dif_amt / pp.scheduledhours) * (pp.scheduledhours * 24)) + pc.compamount,'00000000d00') else null end as f18_o13_ee_annual_salary 
        ...
        (select paymentheaderid from pspay_payment_header where payscheduleperiodid in 
                (select payscheduleperiodid from pspay_payroll_pay_sch_periods where pspaypayrollid in 
                        (select pspaypayrollid from pspay_payroll ppay
                             where ppay.pspaypayrollstatusid = 4 )))  and ppd.etv_id  in  ('EBD','E01','E64','EB5','E05','EB8','E15','E16','E17','E18','E19','E20','ECA','EAJ','EAR','EB4','EAK','ECH','E27','ECJ','E08') --10/13/2020 added E08 RetroPay 
...
where pi.identitytype = 'SSN'
  ...
  --and pe.personid = '63794'
  --and pe.personid in ('68421','63185','66074','65915','65711','68349','68438','65947','67855','68580') --
  order by personid
