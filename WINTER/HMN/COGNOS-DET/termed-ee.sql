select distinct
 pi.personid
,'WINTER TERMED EES' ::CHAR(20) as sourceseq 
...
,case when pc.compevent in ('TempConv','Temp','Hire','ReHire') then ' '
      when (pc.compamount - lastcompdt.increaseamount) <> 0 then to_char(round((lastcompdt.increaseamount / (pc.compamount - lastcompdt.increaseamount)) * 100, 2 ),'0000.00') else '0'
...
,case when pc.compevent in ('TempConv','Temp','Hire','ReHire') then '0' else
      to_char(coalesce((pc.compamount - lastcompdt.increaseamount),0),'00000000d00') end ::char(12) as f24_o21_prev_annual_salary 
...
left join (select a.*
            ...
            join person_employment pe 
             ...
             and pe.emplstatus = 'T' 
           where asofdate = pe.effectivedate - interval '1 day') as vpcpira on vpcpira.personid = pc.personid and vpcpira.earningscode = pc.earningscode and vpcpira.percent_in_range <> 0


----- Position Organization Joins
left join  (select personid, positionid, scheduledhours, schedulefrequency, perspospid, max(effectivedate) as effectivedate, RANK() OVER (PARTITION BY personid ORDER BY max(effectivedate) DESC) AS RANK
              from pers_pos where current_timestamp between createts and endts and effectivedate < enddate
             group by personid, positionid, scheduledhours, schedulefrequency, perspospid) pp on pp.personid = pe.personid and pp.rank = 1
