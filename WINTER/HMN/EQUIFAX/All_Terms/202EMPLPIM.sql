...
select distinct
 ...
 perschkdt.personid
,pe.emplstatus 
,'EE General Information' ::varchar(50) as qsource
...
,case when pe.emplstatus not in ('A','L','P') then to_char(coalesce(pe.paythroughdate,pe.effectivedate),'yyyymmdd') else ' ' end ::char(8) as termdate
,null ::char(1) as termreason
,pe.emplevent
,case when pe.emplstatus not in ('A','L','P') then pe.empleventdetcode else ' ' end ::char(10) as uctermreason
,case when pe.emplstatus not in ('A','L','P') then to_char(coalesce(pe.paythroughdate,pe.effectivedate),'yyyymmdd') else ' ' end ::char(8) as lastdaywrkf
...
join (select personid, emplevent, emplstatus, emplclass, emplpermanency, benefitstatus, emplhiredate, empllasthiredate, effectivedate, enddate, createts, endts, paythroughdate, empleventdetcode, emplservicedate, rank() over (partition by personid order by effectivedate , createts ) as rank  
        ...
        ) pe on pe.personid = perschkdt.personid and pe.rank = 1 
