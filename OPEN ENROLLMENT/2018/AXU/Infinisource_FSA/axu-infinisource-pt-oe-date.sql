...
,pe.emplclass ::char(9) as employee_class --send only FT employees
,case when pu.payunitid = '1' then 'Weekly'
      when pu.payunitid = '2' then 'BiWeekly' 
...
WHERE pi.identitytype = 'SSN'
  ...
  and case when ? = 'OE' then pbe.benefitelection =  'E' and pbe.effectivedate >= date_trunc('year', current_date + interval '1 year') 
           when ? = 'FF' then pbe.benefitelection <> 'W' and current_date between pbe.effectivedate and pbe.enddate AND pbe.benefitelection IN ('T','E') AND pbe.enddate = '2199-12-31' 
           end
