...
LEFT JOIN personbenoptioncostl pbcst ON pbe.personid = pbcst.personid AND pbe.personbeneelectionpid = pbcst.personbeneelectionpid AND pbcst.costsby = 'P'::text
where bpd.edtcode <> '' and ((pbe.enddate > NOW() and pbe.effectivedate <= pbe.enddate) OR (pbe.enddate >= current_date - interval '30 days' and pbe.effectivedate <= pbe.enddate))
